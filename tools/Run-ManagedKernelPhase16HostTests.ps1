...
$out = Join-Path $root 'artifacts\managed-kernel-phase16-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase16HostTests\ManagedKernelPhase16HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 16 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 16 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase16HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 16 host tests failed: $LASTEXITCODE" }
