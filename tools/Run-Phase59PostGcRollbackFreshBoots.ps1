...
Require ($RunCount -ge 1) 'At least one Phase 59 boot is required.'
Require ((Test-Path -LiteralPath $efi) -and (Test-Path -LiteralPath $payload)) 'Phase 59 harness or payload is missing.'
Require ($expectedHash -match '^[0-9A-F]{64}$') 'Payload SHA-256 must be 64 hex characters.'
Require ((Get-FileHash -LiteralPath $payload -Algorithm SHA256).Hash.ToUpperInvariant() -eq $expectedHash) 'Staged Phase 59 payload hash is not authoritative.'
    ...
    for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
        ...
        try {
            ...
            while ((Get-Date) -lt $deadline) {
                ...
                if ($text.Contains('GXOS_NET10:PHASE59_COMPLETE=1') -or $text.Contains('GXOS_NET10:PHASE59_FAILURE=1') -or $text.Contains('GXOS_NET10:FAIL:') -or $text.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=')) { break }
        ...
        Require ($text.Contains('GXOS_NET10:PHASE59_PASS=1') -and !$text.Contains('GXOS_NET10:PHASE59_FAILURE=1') -and !$text.Contains('GXOS_NET10:FAIL:') -and !$text.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and !$text.Contains('GXOS_NET10:PAGE_FAULT_')) "run $sequence did not complete a clean Phase 59 proof"
        foreach ($marker in @(
            'GXOS_NET10:PHASE53O_PASS=1','GXOS_NET10:MANAGED_GC_MAIN_OK=1',
            'GXOS_NET10:PHASE59_BEGIN','GXOS_NET10:PHASE59_INJECTION_POINT=AFTER_GC_ROOT_SURVIVAL',
            ...
            'GXOS_NET10:PHASE59_ROOT_SURVIVAL_BEFORE_FAILURE=1',
            'GXOS_NET10:PHASE59_EXACTLY_ONE_ROOT_RELEASE=1',
            'GXOS_NET10:PHASE59_EXACTLY_ONE_DETACH=1',
            'GXOS_NET10:PHASE59_NO_DOUBLE_CLEANUP=1',
            'GXOS_NET10:PHASE59_GENERATION_SAFE_SLOT_REUSE=1',
            'GXOS_NET10:PHASE59_LEAK_TREND_NONE=1',
            'GXOS_NET10:MANAGED_WORKER_POSTGC_ROLLBACK_OK=1',
            'GXOS_NET10:PHASE59_COMPLETE=1','GXOS_NET10:PHASE59_PASS=1')) {
            Require ($text.Contains($marker)) "run $sequence missing marker: $marker"
        }
        foreach ($marker in @(
            @('GXOS_NET10:PHASE59_INJECTION_FIRED=1',12),
            @('GXOS_NET10:PHASE59_GC_COMPLETED=1',12),
            @('GXOS_NET10:PHASE59_ROOT_SURVIVAL_PROVEN=1',12),
            @('GXOS_NET10:PHASE59_NORMAL_POST_GC_CONTINUATION_SUPPRESSED=1',12),
            @('GXOS_NET10:PHASE59_ROOT_RELEASED=1',12),
            @('GXOS_NET10:PHASE59_DUPLICATE_ROOT_RELEASE_REJECTED=1',12),
            @('GXOS_NET10:PHASE59_DUPLICATE_DETACH_REJECTED=1',12),
            @('GXOS_NET10:PHASE59_FAILED_ROOT_SURVIVED_GC=1',12),
            @('GXOS_NET10:PHASE59_REPLACEMENT_POST_GC_CONTINUATION=1',12),
            @('GXOS_NET10:PHASE59_REPLACEMENT_ROOT_SURVIVED_GC=1',12),
            @('GXOS_NET10:PHASE59_REPLACEMENT_RECLAIM=1',12),
            @('GXOS_NET10:PHASE59_STALE_PRE_GC_OBJECT_IDENTITY_REJECTED=1',12))) {
            Require ((Count-Token $text $marker[0]) -eq $marker[1]) "run $sequence marker count mismatch: $($marker[0])"
        }
        Require ((Get-Hex $text 'GXOS_NET10:PHASE59_INJECTED_FAILURE_CYCLES=') -eq 12 -and
                 (Get-Hex $text 'GXOS_NET10:PHASE59_PASSED_FAILURE_CYCLES=') -eq 12) "run $sequence did not pass all 12 failure cycles"
        Require ((Get-Hex $text 'GXOS_NET10:PHASE59_FAILED_ROOT_RELEASES=') -eq 12 -and
                 (Get-Hex $text 'GXOS_NET10:PHASE59_DUPLICATE_ROOT_RELEASE_REJECTIONS=') -eq 12) "run $sequence root release totals are incorrect"
        $baselineVm = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_BASELINE_VM_REGIONS='; $peakVm = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_PEAK_VM_REGIONS='
        $baselineThreads = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_BASELINE_THREADS='; $peakThreads = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_PEAK_THREADS='
        $baselineObjects = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_BASELINE_OBJECTS='; $peakObjects = Get-Hex $text 'GXOS_NET10:PHASE59_RESOURCE_PEAK_OBJECTS='
        ...
        Write-Output ("PHASE59_POSTGC_ROLLBACK_RUN_{0}=PASS cycles=12 peakVm=0x{1:X} peakThreads=0x{2:X} peakObjects=0x{3:X} serial={4}" -f $sequence,$peakVm,$peakThreads,$peakObjects,$serial)
    }
...
Write-Output "PHASE59_POSTGC_ROLLBACK_PAYLOAD_SHA256=$expectedHash"
Write-Output "PHASE59_POSTGC_ROLLBACK_RUNS=$RunCount"
