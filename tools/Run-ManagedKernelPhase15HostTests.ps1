...
$out = Join-Path $root 'artifacts\managed-kernel-phase15-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase15HostTests\ManagedKernelPhase15HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 15 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 15 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase15HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 15 host tests failed: $LASTEXITCODE" }
