[CmdletBinding()]
param(
    [Parameter(Mandatory = $true)] [string]$GateDirectory,
    [Parameter(Mandatory = $true)] [string]$EvidenceDirectory,
    [Parameter(Mandatory = $true)] [string]$PayloadSha256,
    [Parameter(Mandatory = $true)] [long]$PayloadSize,
    [int]$RunCount = 3,
    [int]$TimeoutSeconds = 600
)

Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'
if ($RunCount -lt 3) { throw 'Three fresh Phase 33 negative boots are required.' }
$runner = Join-Path $PSScriptRoot 'Run-ManagedKernelPhase11FreshBoots.ps1'
$gate = [IO.Path]::GetFullPath($GateDirectory)
$evidence = [IO.Path]::GetFullPath($EvidenceDirectory)
if (Test-Path -LiteralPath $evidence) {
    throw "Evidence directory already exists: $evidence"
}

& $runner -GateDirectory $gate -EvidenceDirectory $evidence `
    -PayloadSha256 $PayloadSha256 -PayloadSize $PayloadSize `
    -RunCount $RunCount -TimeoutSeconds $TimeoutSeconds `
    -EnablePhase15Rx -EnablePhase33Protocol -EnablePhase33NegativeControl `
    -Phase15EnableFilterDump
if ($LASTEXITCODE -ne 0) {
    throw "Phase 33 negative fresh boots failed: $LASTEXITCODE"
}

$runReports = @()
foreach ($serial in @(Get-ChildItem -LiteralPath (Join-Path $evidence 'runs') -Filter serial.log -Recurse)) {
    $text = Get-Content -LiteralPath $serial.FullName -Raw
    foreach ($marker in @(
        'GXOS_NET10:MANAGED_KERNEL_PHASE33_REQUESTED',
        'GXOS_NET10:MANAGED_HTTPS_PHASE33_CHUNKED_SELECTED',
        'GXOS_NET10:MANAGED_KERNEL_PHASE33_START_FAILED')) {
        if (-not $text.Contains($marker)) {
            throw "Phase 33 negative boot missing marker '$marker': $($serial.FullName)"
        }
    }
    if ($text.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE33_PASS') -or
        $text.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE33_PASS') -or
        $text.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -or
        $text.Contains('GXOS_NET10:PAGE_FAULT_') -or
        $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) {
        throw "Phase 33 negative boot emitted success or a machine fault: $($serial.FullName)"
    }
    if (-not $text.Contains('GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof')) {
        throw "Phase 33 negative boot did not report the expected proof rejection: $($serial.FullName)"
    }
    $runReports += "serial=$($serial.FullName) sha256=$((Get-FileHash -LiteralPath $serial.FullName -Algorithm SHA256).Hash.ToUpperInvariant())"
}
if ($runReports.Count -ne $RunCount) {
    throw "Expected $RunCount Phase 33 negative serial logs, found $($runReports.Count)."
}

Set-Content -LiteralPath (Join-Path $evidence 'phase33-negative-summary.log') -Value @(
    'MANAGED_KERNEL_PHASE33_NEGATIVE_CONTROL=PASS',
    "MANAGED_KERNEL_PHASE33_NEGATIVE_RUNS=$RunCount",
    "MANAGED_KERNEL_PHASE33_PAYLOAD_SHA256=$($PayloadSha256.ToUpperInvariant())",
    "MANAGED_KERNEL_PHASE33_PAYLOAD_SIZE=$PayloadSize",
    'MANAGED_KERNEL_PHASE33_NEGATIVE_FAULT=MALFORMED_LATER_CHUNK',
    'MANAGED_KERNEL_PHASE33_NEGATIVE_EXPECTED=PHASE33_START_REJECTED_NO_SUCCESS',
    $runReports) -Encoding ascii
Get-Content -LiteralPath (Join-Path $evidence 'phase33-negative-summary.log')
