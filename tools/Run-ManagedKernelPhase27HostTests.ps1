...
$out = Join-Path $root 'artifacts\managed-kernel-phase27-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase27HostTests\ManagedKernelPhase27HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 27 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 27 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase27HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 27 host tests failed: $LASTEXITCODE" }
