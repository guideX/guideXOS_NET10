...
$out = Join-Path $root 'artifacts\managed-kernel-phase17-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase17HostTests\ManagedKernelPhase17HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 17 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 17 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase17HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 17 host tests failed: $LASTEXITCODE" }
