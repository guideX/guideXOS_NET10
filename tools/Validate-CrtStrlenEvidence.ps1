...
if ($failures.Count -eq 0) {
    ...
    foreach ($run in $runRecords) {
        ...
        if ((Read-Decimal $text 'GXOS_NET10:PE_IMPORT_FUNCTIONAL=') -ne $(if ($Mode -eq 'Positive') { 27 } else { 26 }) -or
            (Read-Decimal $text 'GXOS_NET10:PE_IMPORT_FAILFAST=') -ne $(if ($Mode -eq 'Positive') { 97 } else { 98 }) -or
        ...
        if ($Mode -eq 'Disabled') {
            if (Has-Text $text 'GXOS_NET10:CRT_STRLEN_BEGIN' -or Has-Text $text 'GXOS_NET10:CRT_STRLEN_OK') {
                Add-Failure "disabled strlen implementation executed: $runId"
            }
            if (-not (Has-Text $text 'GXOS_NET10:CRT_STRCMP_RESULT=0x0000000000000001')) {
                Add-Failure "disabled control did not preserve strcmp: $runId"
            }
            continue
        }
        ...
        Require-Ordered $text @(
            ...
            'GXOS_NET10:CRT_STRLEN_BEGIN',
            'GXOS_NET10:CRT_STRLEN_STATUS=0x0000000000000000',
            'GXOS_NET10:CRT_STRLEN_RETURNED',
            'GXOS_NET10:CRT_STRLEN_OK',
            'GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetEnvironmentVariableW'
        ) $runId
        if ((Read-Last-Hex $text 'GXOS_NET10:CRT_STRLEN_CALL_COUNT=0x') -ne 1 -or
            (Read-Last-Hex $text 'GXOS_NET10:CRT_STRLEN_SUCCESS_COUNT=0x') -ne 1 -or
            (Read-Last-Hex $text 'GXOS_NET10:CRT_STRLEN_FAILURE_COUNT=0x') -ne 0 -or
            (Read-Last-Hex $text 'GXOS_NET10:CRT_STRLEN_TOTAL_BYTES=0x') -ne 8 -or
            (Read-Last-Hex $text 'GXOS_NET10:CRT_STRLEN_LONGEST=0x') -ne 8 -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_LENGTH=0x') -ne 8 -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_RETURN_VALUE=0x') -ne 8) {
            Add-Failure "strlen result census invalid: $runId"
        }
        if (-not (Has-Text $text 'GXOS_NET10:CRT_STRLEN_BYTES=6763536572766572') -or
            -not (Has-Text $text 'GXOS_NET10:CRT_STRLEN_PREVIEW="gcServer"') -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_REGION_READABLE=0x') -ne 1 -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_REGION_EXECUTABLE=0x') -ne 0 -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_REGION_WRITABLE=0x') -ne 0 -or
            (Read-Hex $text 'GXOS_NET10:CRT_STRLEN_RELOCATIONS_APPLIED=0x') -ne 1) {
            Add-Failure "strlen bounded diagnostics invalid: $runId"
        }
        if ((Read-Hex $text 'GXOS_NET10:CRT_STRLEN_POINTER=0x') -ne
            (Read-Hex $text 'GXOS_NET10:CRT_STRCMP_LHS_POINTER=0x')) {
            Add-Failure "strlen pointer did not equal strcmp operand: $runId"
        }
    }
}
...
Write-Output 'CRT_STRLEN_EVIDENCE_VALIDATION=PASSED'
