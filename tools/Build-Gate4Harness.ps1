...
param(
    ...
    [switch]$EnableManagedKernelPhase33,
    [switch]$EnableManagedKernelPhase34,
    [switch]$EnableManagedKernelPhase35,
    [switch]$EnableManagedKernelPhase39,
    [switch]$EnableManagedKernelPhase40,
    [switch]$EnableManagedKernelPhase41,
    [switch]$EnableManagedKernelPhase42,
    [switch]$EnableManagedKernelPhase43,
    [switch]$EnableManagedKernelPhase43Capacity,
    [switch]$EnableManagedKernelPhase44,
    [switch]$EnableManagedKernelPhase44Capacity,
    [switch]$EnableManagedKernelPhase45,
    [switch]$EnableManagedKernelPhase45Capacity,
    [switch]$EnableManagedKernelPhase46,
    [switch]$EnableManagedKernelPhase46Capacity,
    [switch]$EnableManagedKernelPhase48,
    [switch]$EnableManagedKernelPhase49,
    [switch]$EnableManagedKernelPhase50,
    [switch]$EnableManagedKernelPhase51,
    [switch]$EnableManagedKernelPhase51ResetReuse,
    [switch]$EnableManagedKernelPhase52,
    [switch]$EnableManagedKernelPhase53,
...
$phase58PayloadSha256 = '1214CB1178376ED4AC43B2F178B81B6CD3E809C18B33CABC7B6293AEEDD60300'
$phase58PayloadSize = 732672
$phase59PayloadSha256 = '2E25F807194864CF667FA2B10D72D608247518D3C4A2C02DF691A9D7FCB16FCC'
$phase59PayloadSize = 732160
...
if ($EnableNativeAotManagedWorkerApi -and -not $EnableNativeAotSchedulerThreadLifecycle) {
    throw 'Phase 61 managed worker API requires -EnableNativeAotSchedulerThreadLifecycle.'
}
if ($EnableNativeAotManagedWorkerApi -and -not $EnableNativeAotStartup) {
    throw 'Phase 61 managed worker API requires -EnableNativeAotStartup.'
}
if ($EnableNativeAotManagedWorkerApi -and -not $EnableNativeAotManagedCallback) {
    throw 'Phase 61 managed worker API requires -EnableNativeAotManagedCallback.'
}
if ($EnableNativeAotManagedWorkerApi -and -not $EnableNativeAotSchedulerCallback) {
    throw 'Phase 61 managed worker API requires -EnableNativeAotSchedulerCallback.'
}
...
if ($EnablePhase57PostAttachRollback -and $Scenario -ne 'NativeAotEventWait') {
    throw 'Phase 57 post-attach rollback validation requires the NativeAotEventWait scenario.'
}
if ($EnablePhase57PostAttachRollback -and -not $EnableNativeAotStartup) {
    throw 'Phase 57 post-attach rollback validation requires -EnableNativeAotStartup.'
}
if ($EnablePhase57PostAttachRollback -and
    -not $EnableNativeAotManagedCallback) {
    throw 'Phase 57 post-attach rollback validation requires -EnableNativeAotManagedCallback.'
}
if ($EnablePhase57PostAttachRollback -and
    -not $EnableNativeAotSchedulerCallback) {
    throw 'Phase 57 post-attach rollback validation requires -EnableNativeAotSchedulerCallback.'
}
if ($EnablePhase57PostAttachRollback -and
    -not $EnableNativeAotManagedGcProbe) {
    throw 'Phase 57 post-attach rollback validation requires -EnableNativeAotManagedGcProbe.'
}
if ($EnablePhase57PostAttachRollback -and
    -not $EnableNativeAotSchedulerThreadLifecycle) {
    throw 'Phase 57 post-attach rollback validation requires -EnableNativeAotSchedulerThreadLifecycle.'
}
if ($EnablePhase58PostRootRollback -and $Scenario -ne 'NativeAotEventWait') {
    throw 'Phase 58 post-root rollback validation requires the NativeAotEventWait scenario.'
}
if ($EnablePhase58PostRootRollback -and -not $EnableNativeAotStartup) {
    throw 'Phase 58 post-root rollback validation requires -EnableNativeAotStartup.'
}
if ($EnablePhase58PostRootRollback -and -not $EnableNativeAotManagedCallback) {
    throw 'Phase 58 post-root rollback validation requires -EnableNativeAotManagedCallback.'
}
if ($EnablePhase58PostRootRollback -and -not $EnableNativeAotSchedulerCallback) {
    throw 'Phase 58 post-root rollback validation requires -EnableNativeAotSchedulerCallback.'
}
if ($EnablePhase58PostRootRollback -and -not $EnableNativeAotManagedGcProbe) {
    throw 'Phase 58 post-root rollback validation requires -EnableNativeAotManagedGcProbe.'
}
if ($EnablePhase58PostRootRollback -and -not $EnableNativeAotSchedulerThreadLifecycle) {
    throw 'Phase 58 post-root rollback validation requires -EnableNativeAotSchedulerThreadLifecycle.'
}
if ($EnablePhase59PostGcRollback -and $Scenario -ne 'NativeAotEventWait') {
    throw 'Phase 59 post-GC rollback validation requires the NativeAotEventWait scenario.'
}
if ($EnablePhase59PostGcRollback -and -not $EnableNativeAotStartup) {
    throw 'Phase 59 post-GC rollback validation requires -EnableNativeAotStartup.'
}
if ($EnablePhase59PostGcRollback -and -not $EnableNativeAotManagedCallback) {
    throw 'Phase 59 post-GC rollback validation requires -EnableNativeAotManagedCallback.'
}
if ($EnablePhase59PostGcRollback -and -not $EnableNativeAotSchedulerCallback) {
    throw 'Phase 59 post-GC rollback validation requires -EnableNativeAotSchedulerCallback.'
}
if ($EnablePhase59PostGcRollback -and -not $EnableNativeAotManagedGcProbe) {
    throw 'Phase 59 post-GC rollback validation requires -EnableNativeAotManagedGcProbe.'
}
...
$managedWorkerApiSource = Join-Path $root 'src\Gate4Harness\nativeaot_managed_worker_api.c'
...
if ($Scenario -eq 'CreateEventW' -or $Scenario -eq 'CreateEventWDisabled' -or
    ...
    if ($PayloadMode -eq 'ManagedKernel') {
        ...
        if ($payloadHash -ne $phase58PayloadSha256 -or
            $payloadSize -ne $phase58PayloadSize) {
            throw "Phase 58 requires the captured post-root payload. Hash=$payloadHash Size=$payloadSize"
        }
    }
...
if ($EnablePhase57PostAttachRollback) {
    # Reuse the Phase 56 single-shot, generation-aware injection mechanism;
    # the Phase 57 point is the only executable point in this configuration.
    $gccArguments += '-DGXOS_ENABLE_PHASE56_FAILURE_INJECTION'
    $gccArguments += '-DGXOS_ENABLE_PHASE57_POSTATTACH_ROLLBACK'
}
if ($EnablePhase58PostRootRollback) {
    # Reuse the Phase 56 single-shot, generation-aware injection mechanism;
    # the Phase 58 point is the only executable point in this configuration.
    $gccArguments += '-DGXOS_ENABLE_PHASE56_FAILURE_INJECTION'
    $gccArguments += '-DGXOS_ENABLE_PHASE58_POSTROOT_ROLLBACK'
}
if ($EnablePhase59PostGcRollback) {
    # Reuse the Phase 56 single-shot, generation-aware injection mechanism;
...
if ($PayloadMode -eq 'ManagedKernel' -and
    $Scenario -in @('ManagedKernelPhase26', 'ManagedKernelPhase29', 'ManagedKernelPhase30', 'ManagedKernelPhase31', 'ManagedKernelPhase32', 'ManagedKernelPhase33', 'ManagedKernelPhase34', 'ManagedKernelPhase35', 'ManagedKernelPhase39', 'ManagedKernelPhase40', 'ManagedKernelPhase41', 'ManagedKernelPhase42', 'ManagedKernelPhase43', 'ManagedKernelPhase44', 'ManagedKernelPhase45', 'ManagedKernelPhase46', 'ManagedKernelPhase48', 'ManagedKernelPhase49', 'ManagedKernelPhase50', 'ManagedKernelPhase51', 'ManagedKernelPhase51ResetReuse', 'ManagedKernelPhase52', 'ManagedKernelPhase53')) {
    $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE11'
    if ($Scenario -eq 'ManagedKernelPhase26') {
    ...
    if ($EnableManagedKernelPhase33) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE33'
    }
    if ($EnableManagedKernelPhase34) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE34'
    }
    if ($EnableManagedKernelPhase35) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE35'
    }
    if ($EnableManagedKernelPhase39) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE39'
    }
    if ($EnableManagedKernelPhase40) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE40'
    }
    if ($EnableManagedKernelPhase41) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE41'
    }
    if ($EnableManagedKernelPhase42) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE42'
    }
    if ($EnableManagedKernelPhase43) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE43'
    }
    if ($EnableManagedKernelPhase43Capacity) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE43_CAPACITY'
    }
    if ($EnableManagedKernelPhase44) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE44'
    }
    if ($EnableManagedKernelPhase44Capacity) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE44_CAPACITY'
    }
    if ($EnableManagedKernelPhase45) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE45'
    }
    if ($EnableManagedKernelPhase45Capacity) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE45_CAPACITY'
    }
    if ($EnableManagedKernelPhase46) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE46'
    }
    if ($EnableManagedKernelPhase46Capacity) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE46_CAPACITY'
    }
    if ($EnableManagedKernelPhase48) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE48'
    }
    if ($EnableManagedKernelPhase49) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE49'
    }
    if ($EnableManagedKernelPhase50) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE50'
    }
    if ($EnableManagedKernelPhase51) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE51'
    }
    if ($EnableManagedKernelPhase51ResetReuse) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE51_RESET_REUSE'
    }
    if ($EnableManagedKernelPhase52) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE52'
    }
    if ($EnableManagedKernelPhase53) {
        $gccArguments += '-DGXOS_ENABLE_MANAGED_KERNEL_PHASE53'
    }
}
