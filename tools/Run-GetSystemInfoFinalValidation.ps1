...
$validatorScript = Join-Path $root 'tools\Validate-GetSystemInfoEvidence.ps1'
...
foreach ($path in @($efi,$payload,$startup,$payloadSource,$runtimeArchive,$validatorScript,$ovmf,$varsTemplate) + $sourceFiles) {
...
function Snapshot([string]$kind, [string]$path) {
    ...
    [PSCustomObject]@{ Kind=$kind; Path=[IO.Path]::GetFullPath($path); Sha256=(Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash.ToUpperInvariant(); Length=[int64]$item.Length; LastWriteTimeUtc=$item.LastWriteTimeUtc.ToString('o') }
}
...
$expectedCompleteMarker = if ($Mode -eq 'MarkerMutation') { 'GXOS_NET10:GETSYSTEMINFO_OX' } elseif ($Mode -eq 'Disabled') { 'GXOS_NET10:GETSYSTEMINFO_FAILFAST_RCX=0x' } else { 'GXOS_NET10:GETSYSTEMINFO_OK' }
...
function Assert-ArtifactSet($expected, [string]$phase) {
    foreach ($artifact in $expected) {
        ...
        if ($actualHash -ne $artifact.Sha256 -or [int64]$item.Length -ne [int64]$artifact.Length) {
            throw "Immutable artifact changed during ${phase}: $($artifact.Path)"
        }
    }
}
...
for ($sequence=1; $sequence -le $RunCount; $sequence++) {
    ...
    $process=Start-Process -FilePath $qemu -ArgumentList $args -WorkingDirectory $gate -RedirectStandardOutput $stdout -RedirectStandardError $stderr -PassThru -WindowStyle Hidden; $qemuPid=[int]$process.Id; $start=(Get-Date).ToUniversalTime(); Write-Event $events 'qemu-started' @{RunId=$runId;Pid=$qemuPid;StartUtc=$start.ToString('o');Arguments=($args -join ' ')}
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete=if ($Mode -eq 'Disabled') { $text.Contains($expectedCompleteMarker) } else { $text.Contains('GXOS_NET10:GETSYSTEMINFO_FIELD_CONSUMPTION_COMPLETE') -and $text.Contains($expectedCompleteMarker) }
    ...
    $finalComplete=if ($Mode -eq 'Disabled') { $final.Contains($expectedCompleteMarker) } else { $final.Contains('GXOS_NET10:GETSYSTEMINFO_FIELD_CONSUMPTION_COMPLETE') -and $final.Contains($expectedCompleteMarker) }
...
Write-Output "GETSYSTEMINFO_EVIDENCE_ROOT=$evidence"
Write-Output 'GETSYSTEMINFO_FINAL_VALIDATION=PASSED'
