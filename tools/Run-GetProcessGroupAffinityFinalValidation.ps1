...
$validatorScript = Join-Path $root 'tools\Validate-GetProcessGroupAffinityEvidence.ps1'
...
for ($sequence=1; $sequence -le $RunCount; $sequence++) {
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete=$text.Contains("GXOS_NET10:UNEXPECTED_IMPORT_CALL:$expectedBoundary")
        if($summary -and $complete){$captured=$true;Write-Event $events 'complete-evidence-captured' @{RunId=$runId;SerialLength=$text.Length};Start-Sleep -Milliseconds 500;break}
    ...
    if ($finalSummary -and $finalBoundary) { $captured=$true; Write-Event $events 'complete-evidence-captured-after-cleanup' @{RunId=$runId;SerialLength=$final.Length} }
...
Write-Output "GETPROCESSGROUPAFFINITY_EVIDENCE_ROOT=$evidence"
Write-Output 'GETPROCESSGROUPAFFINITY_FINAL_VALIDATION=PASSED'
