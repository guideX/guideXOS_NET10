...
$validatorScript = Join-Path $root 'tools\Validate-GetEnvironmentVariableWEvidence.ps1'
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        if ($matches.Count -ne 0) {
            $boundary = $matches[$matches.Count - 1].Groups[1].Value
            $boundaryObserved = if ($Mode -eq 'Positive') { $text.Contains('GXOS_NET10:GETENV_OK') } else { $boundary -eq 'KERNEL32.dll!GetEnvironmentVariableW' }
        }
        $summaryObserved = $text.Contains('GXOS_NET10:QPC_REGRESSIONS=') -and $text.Contains('GXOS_NET10:ALLOCATION_CONTEXT_VALID=0')
        if ($Mode -eq 'Positive') { $summaryObserved = $summaryObserved -and $text.Contains('GXOS_NET10:GETENV_OK') }
...
Write-Output "GETENV_EVIDENCE_ROOT=$evidence"
Write-Output 'GETENV_FINAL_VALIDATION=PASSED'
