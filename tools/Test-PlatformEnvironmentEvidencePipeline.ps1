...
if (-not (Test-Path -LiteralPath $positive) -or -not (Test-Path -LiteralPath $disabled)) {
    throw 'Both positive and disabled evidence roots are required.'
}
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $root 'evidence\generated\getenv-negative-controls-20260731'
}
$OutputRoot = [IO.Path]::GetFullPath($OutputRoot)
New-Item -ItemType Directory -Force -Path $OutputRoot | Out-Null
$validator = Join-Path $root 'tools\Validate-GetEnvironmentVariableWEvidence.ps1'
...
function Expect-Rejection([string]$name, [string]$path, [string]$expectedText) {
    ...
    Write-Output "GETENV_NEGATIVE_${name}=PASS"
}
...
Write-Output 'GETENV_NEGATIVE_DISABLED_ROUTING=PASS'

$case = Copy-Case 'marker-mutation'
$serial = Join-Path $case 'runs\run-1\serial.log'
$text = [IO.File]::ReadAllText($serial)
$text = $text.Replace('GXOS_NET10:GETENV_OK', 'GXOS_NET10:GETENV_OX')
[IO.File]::WriteAllText($serial, $text)
Expect-Rejection 'MARKER_MUTATION' $case 'missing or out of order: GXOS_NET10:GETENV_OK'
...
Write-Output 'GETENV_NEGATIVE_EVIDENCE_PIPELINE=PASSED'
