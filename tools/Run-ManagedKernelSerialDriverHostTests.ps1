...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $root 'artifacts\managed-kernel-serial-driver-host-tests'
}
$out = [IO.Path]::GetFullPath($OutputDirectory)
$project = Join-Path $root 'src\ManagedKernelSerialDriverHostTests\ManagedKernelSerialDriverHostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "serial driver host restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "serial driver host build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelSerialDriverHostTests.dll'
if (-not (Test-Path -LiteralPath $assembly)) { throw "serial driver host assembly missing: $assembly" }
& $dotnet.Source $assembly 1> (Join-Path $out 'run.stdout.log') `
    2> (Join-Path $out 'run.stderr.log')
if ($LASTEXITCODE -ne 0) { throw "ManagedKernel serial driver host tests failed: $LASTEXITCODE" }
