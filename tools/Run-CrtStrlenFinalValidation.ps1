...
$validatorScript = Join-Path $root 'tools\Validate-CrtStrlenEvidence.ps1'
...
function Test-QemuGone([int]$processId) {
    try {
        ...
        return $process.ProcessName -ne 'qemu-system-x86_64'
    } catch { return $true }
}
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    $boundary = if ($Mode -eq 'Positive') { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetEnvironmentVariableW' } else { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-string-l1-1-0.dll!strlen' }
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        if ($Mode -eq 'Positive') { $summaryObserved = $summaryObserved -and $text.Contains('GXOS_NET10:CRT_STRLEN_OK') }
    ...
    if ($alive) {
        Write-Event $events 'qemu-stop-requested' @{ RunId = $runId; Pid = $processId; Reason = if ($captured) { 'capture-complete' } else { 'incomplete-evidence' } }
        Stop-Process -Id $processId -Force -ErrorAction SilentlyContinue
    }
    $cleanupComplete = Wait-QemuGone $processId 15
    ...
    $runData = [ordered]@{
        ...
        LastObservedGuestMarker = $boundary; FinalSerialLength = if ($null -eq $serialItem) { 0 } else { [int64]$serialItem.Length }
        ...
        HarnessEventLog = $events.Substring($evidence.Length).TrimStart('\','/'); VarsPath = $vars.Substring($evidence.Length).TrimStart('\','/')
    }
...
Write-Output "CRT_STRLEN_EVIDENCE_ROOT=$evidence"
Write-Output 'CRT_STRLEN_FINAL_VALIDATION=PASSED'
