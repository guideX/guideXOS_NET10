...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "interrupt host restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "interrupt host build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
...
if ($LASTEXITCODE -ne 0) { throw "ManagedKernel interrupt host tests failed: $LASTEXITCODE" }
