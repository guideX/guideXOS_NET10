...
$validatorScript = Join-Path $root 'tools\Validate-GetProcessAffinityMaskEvidence.ps1'
$runnerScript = [IO.Path]::GetFullPath($PSCommandPath)
$sourceFiles = @(
    (Join-Path $root 'src\Gate4Harness\platform_process_affinity.c'),
    (Join-Path $root 'src\Gate4Harness\platform_process_affinity.h'),
    (Join-Path $root 'src\Gate4Harness\platform_process_group_affinity.c'),
    (Join-Path $root 'src\Gate4Harness\platform_process_group_affinity.h'),
...
foreach ($path in @($efi,$payload,$startup,$payloadSource,$runtimeArchive,$validatorScript) + $sourceFiles + @($ovmf,$varsTemplate)) {
...
$artifacts = @(
    ...
    (Snapshot 'nativeaot_payload_source' $payloadSource), (Snapshot 'runtime_archive' $runtimeArchive),
    (Snapshot 'ovmf_code' $ovmf), (Snapshot 'ovmf_vars_template' $varsTemplate),
    (Snapshot 'esp_startup' $startup), (Snapshot 'qemu_executable' $qemu),
    (Snapshot 'validation_runner' $runnerScript), (Snapshot 'evidence_validator' $validatorScript)
)
...
Write-Json (Join-Path $evidence 'artifact-manifest.json') ([ordered]@{
    ...
    GateDirectory=$gate; Mode=$Mode; ExpectedBoundary='AUTO'; QemuVersion=$qemuVersion; Artifacts=$artifacts
})
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete = if ($Mode -eq 'Disabled') {
            $summary -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcessAffinityMask')
        } else {
            $summary -and $text.Contains('GXOS_NET10:GETPROCESSAFFINITYMASK_CALLER_CONSUMPTION_COMPLETE') -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')
        }
    ...
    $finalComplete = if ($Mode -eq 'Disabled') { $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcessAffinityMask') } else { $final.Contains('GXOS_NET10:GETPROCESSAFFINITYMASK_CALLER_CONSUMPTION_COMPLETE') -and $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:') }
...
Write-Output "GETPROCESSAFFINITYMASK_EVIDENCE_ROOT=$evidence"
Write-Output 'GETPROCESSAFFINITYMASK_FINAL_VALIDATION=PASSED'
