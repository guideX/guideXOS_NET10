...
$out = Join-Path $root 'artifacts\managed-kernel-phase29-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase29HostTests\ManagedKernelPhase29HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 29 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 29 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase29HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 29 host tests failed: $LASTEXITCODE" }
