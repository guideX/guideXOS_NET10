...
$out = Join-Path $root 'artifacts\managed-kernel-phase30-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase30HostTests\ManagedKernelPhase30HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 30 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 30 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase30HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 30 host tests failed: $LASTEXITCODE" }
