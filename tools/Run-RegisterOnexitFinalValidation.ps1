...
$sourceFiles = @(
    ...
    (Join-Path $root 'src\Gate4Harness\crt_onexit.h'),
    (Join-Path $root 'src\Gate4Harness\tests\crt_onexit_register_tests.c'),
...
function Read-Serial([string]$path) {
    ...
    for ($attempt = 0; $attempt -lt 5; $attempt++) {
        try { return [IO.File]::ReadAllText($path) } catch { Start-Sleep -Milliseconds 50 }
    }
    return ''
}
...
Write-Json (Join-Path $evidence 'validation-context.json') ([ordered]@{
    EvidenceId=$evidenceId; Mode=$Mode; RunCount=$RunCount; TimeoutSeconds=$TimeoutSeconds; NoProgressSeconds=$NoProgressSeconds
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    Write-Event $events 'run-prepared' @{RunId=$runId;Sequence=$sequence;VarsSha256=(Get-FileHash $vars -Algorithm SHA256).Hash.ToUpperInvariant()}
    $quote = [char]34
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete = if ($Mode -eq 'Disabled') {
            $summary -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-runtime-l1-1-0.dll!_register_onexit_function')
        } else {
            $summary -and $text.Contains('GXOS_NET10:REGISTER_ONEXIT_RETURNED')
        }
    ...
    $finalComplete = if ($Mode -eq 'Disabled') { $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-runtime-l1-1-0.dll!_register_onexit_function') } else { $final.Contains('GXOS_NET10:REGISTER_ONEXIT_RETURNED') }
    if ($finalComplete) { $captured=$true; Write-Event $events 'complete-evidence-captured-after-cleanup' @{RunId=$runId;SerialLength=$final.Length} }
    $exit = $null
