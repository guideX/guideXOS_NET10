...
try {
    for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
        ...
        foreach ($marker in @('GXOS_NET10:NATIVEAOT_STARTUP_OK', 'GXOS_NET10:GC_STARTUP_ADVANCED', 'GXOS_NET10:MANAGED_ENTRY_COMPLETE', 'GXOS_NET10:PHASE53O_BEGIN', 'GXOS_NET10:PHASE53O_ATTACH_PATH=GENERATED_REVERSE_PINVOKE', 'GXOS_NET10:PHASE53O_DETACH_PATH=RUNTIME_FLS_FIBER_DETACH_CALLBACK', 'GXOS_NET10:MANAGED_THREAD_ATTACH_OK=1', 'GXOS_NET10:PHASE53O_FLS_CALLBACK_RETURNED=1', 'GXOS_NET10:MANAGED_CALLBACK_RETURN_OK=1', 'GXOS_NET10:MANAGED_GC_ROOT_SURVIVED=1', 'GXOS_NET10:MANAGED_GC_COLLECTION_OBSERVED=1', 'GXOS_NET10:MANAGED_GC_WORKER_RETURN_OK=1', 'GXOS_NET10:MANAGED_THREAD_MANAGED_RETURN_OK=1', 'GXOS_NET10:MANAGED_THREAD_DETACH_OK=1', 'GXOS_NET10:MANAGED_THREAD_UNREGISTER_OK=1', 'GXOS_NET10:PHASE53O_SCHEDULER_RECLAIM_OK=1', 'GXOS_NET10:PHASE53V_WORKER_CONTEXT_COHERENT_ID=0x', 'GXOS_NET10:PHASE53O_REPEAT_OK=1', 'GXOS_NET10:PHASE53O_COMPLETE=1', 'GXOS_NET10:PHASE53O_PASS=1')) {
        ...
        Require ((Get-Hex $text 'GXOS_NET10:PHASE53O_RUNTIME_STACK_LOW=') + (Get-Hex $text 'GXOS_NET10:PHASE53V_GUARD_BYTES=') -eq (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_BASE=') -and (Get-Hex $text 'GXOS_NET10:PHASE53O_RUNTIME_STACK_HIGH=') -eq (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_LIMIT=') -and (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_RSP=') -ge (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_BASE=') -and (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_RSP=') -le (Get-Hex $text 'GXOS_NET10:PHASE53O_STACK_LIMIT=')) "run $sequence runtime-reported stack bounds are incorrect"
        ...
        Require ((Count-Token $text 'GXOS_NET10:PHASE53V_WORKER_CONTEXT_COHERENT_ID=0x') -ge 2) "run $sequence did not prove two-worker RSP/GS coherency"
        Require ((Get-Hex $text 'GXOS_NET10:PHASE53V_WORKER_GS_LOWER=') -eq (Get-Hex $text 'GXOS_NET10:PHASE53V_WORKER_USABLE_LOW=') -and (Get-Hex $text 'GXOS_NET10:PHASE53V_WORKER_TEB_LOWER=') -eq (Get-Hex $text 'GXOS_NET10:PHASE53V_WORKER_USABLE_LOW=')) "run $sequence GS/TEB lower-bound aliasing failed"
        Require ((Get-Hex $text 'GXOS_NET10:PHASE53V_WORKER_HIGH_WATER_BYTES=') -le 0x10000) "run $sequence high-water exceeded the approved initial usable capacity"
