...
$out = Join-Path $root 'artifacts\managed-kernel-phase25-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase25HostTests\ManagedKernelPhase25HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 25 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 25 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase25HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 25 host tests failed: $LASTEXITCODE" }
