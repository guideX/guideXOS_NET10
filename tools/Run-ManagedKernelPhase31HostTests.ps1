...
$out = Join-Path $root 'artifacts\managed-kernel-phase31-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase31HostTests\ManagedKernelPhase31HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 31 host-test restore failed: $LASTEXITCODE" }
    & $dotnet.Source $msbuild $project '/t:Build' '/p:Configuration=Release' `
        '/p:RestoreIgnoreFailedSources=true' `
        "/p:OutputPath=$out\bin\" `
        1> (Join-Path $out 'build.stdout.log') 2> (Join-Path $out 'build.stderr.log')
    if ($LASTEXITCODE -ne 0) { throw "Phase 31 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase31HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 31 host tests failed: $LASTEXITCODE" }
