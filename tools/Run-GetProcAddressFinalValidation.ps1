...
$runtimeArchive = if ([string]::IsNullOrWhiteSpace($RuntimeArchivePath)) {
    Join-Path $root 'artifacts\allocation-enabled-final-20260728-060439-726\static\gxos-managed-entry-probe.lib'
...
$sourceFiles = @(
    ...
    (Join-Path $root 'src\Gate4Harness\platform_get_proc_address.h'),
    (Join-Path $root 'src\Gate4Harness\platform_get_module_handle.h'),
    (Join-Path $root 'src\Gate4Harness\crt_initterm.h'),
...
foreach ($path in @($efi,$payload,$startup,$runtimeArchive,$validator,$runner) + $sourceFiles + @($ovmf,$varsTemplate)) {
...
Write-Json (Join-Path $evidence 'artifact-manifest.json') ([ordered]@{
    ...
    GateDirectory=$gate; Mode=$Mode; ExpectedBoundary=if ($Mode -eq 'Positive') { 'api-ms-win-crt-runtime-l1-1-0.dll!_register_onexit_function' } else { 'KERNEL32.dll!GetProcAddress' }
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    $qemuPidValue = [int]$process.Id
    $start = (Get-Date).ToUniversalTime()
    Write-Event $events 'qemu-started' @{RunId=$runId;Pid=$qemuPidValue;StartUtc=$start.ToString('o');Arguments=($args -join ' ')}
    $captured = $false
    $timeoutReason = ''
    $lastLength = 0
    $lastProgress = $start
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete = if ($Mode -eq 'Disabled') {
            $summary -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcAddress')
        } else {
            $summary -and $text.Contains('GXOS_NET10:GETPROCADDRESS_CALLER_CONSUMPTION_COMPLETE') -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-runtime-l1-1-0.dll!_register_onexit_function')
        }
    ...
    $cleanup = Wait-QemuGone $qemuPidValue
    Start-Sleep -Milliseconds 300
    $final = Read-Serial $serial
    $finalComplete = if ($Mode -eq 'Disabled') { $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcAddress') } else { $final.Contains('GXOS_NET10:GETPROCADDRESS_CALLER_CONSUMPTION_COMPLETE') -and $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-runtime-l1-1-0.dll!_register_onexit_function') }
    if ($final.Contains('GXOS_NET10:QPC_REGRESSIONS=') -and $finalComplete) { $captured=$true; Write-Event $events 'complete-evidence-captured-after-cleanup' @{RunId=$runId;SerialLength=$final.Length} }
    $exit = $null
    try { $process.Refresh(); if ($process.HasExited) { $exit=[int]$process.ExitCode } } catch { }
    ...
    Write-Json (Join-Path $runDirectory 'run.json') ([ordered]@{
        EvidenceId=$evidenceId;RunId=$runId;Sequence=$sequence;Mode=$Mode;Pass=$captured -and $cleanup;QemuPid=$qemuPidValue;QemuVersion=$qemuVersion
        QemuStartUtc=$start.ToString('o');QemuEndUtc=(Get-Date).ToUniversalTime().ToString('o');QemuExitCode=$exit;ProcessExitedNaturally=(-not $alive)
        CleanupComplete=$cleanup;TimeoutReason=$timeoutReason;FinalSerialLength=[int64](Get-Item -LiteralPath $serial).Length;SerialSha256=$serialHash
        ArtifactFingerprint=$artifactFingerprint;SerialLog=('runs\run-'+$sequence+'\serial.log');QemuStdoutLog=('runs\run-'+$sequence+'\qemu.stdout.log')
        QemuStderrLog=('runs\run-'+$sequence+'\qemu.stderr.log');HarnessEventLog=('runs\run-'+$sequence+'\harness-events.jsonl');VarsPath=('runs\run-'+$sequence+'\ovmf-vars.fd')
    })
...
Write-Output "GETPROCADDRESS_EVIDENCE_ROOT=$evidence"
Write-Output 'GETPROCADDRESS_FINAL_VALIDATION=PASSED'
