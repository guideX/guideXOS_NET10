...
$validatorScript = Join-Path $root 'tools\Validate-CrtInittermEvidence.ps1'
$runnerScript = [IO.Path]::GetFullPath($PSCommandPath)
$runsRoot = Join-Path $evidence 'runs'
foreach ($path in @($efi,$payload,$startup,$payloadSource,$runtimeArchive,$validatorScript)) {
...
function Assert-ArtifactSet($expected) {
    ...
    foreach ($item in $expected) {
        ...
        if ($null -eq $now -or $now.Sha256 -ne $item.Sha256 -or
            $now.Length -ne $item.Length -or $now.LastWriteTimeUtc -ne $item.LastWriteTimeUtc) {
            throw "Execution artifact changed: $($item.Kind)"
        }
    }
    return $current
}
function Last-Marker([string]$text) {
    $markers = @('GXOS_NET10:CRT_INITTERM_OK','GXOS_NET10:CRT_INITTERM_BEGIN',
                 'GXOS_NET10:CRT_INITTERM_E_OK','GXOS_NET10:UNEXPECTED_IMPORT_CALL:',
                 'GXOS_NET10:QPC_REGRESSIONS=')
    $last = 'none'; $position = -1
    foreach ($marker in $markers) {
        $at = $text.LastIndexOf($marker, [StringComparison]::Ordinal)
        if ($at -gt $position) { $position = $at; $last = $marker }
    }
    return $last
}
...
Write-JsonFile (Join-Path $evidence 'artifact-manifest.json') ([ordered]@{
    ...
    RepositoryRoot = $root; GateDirectory = $gate; Mode = $Mode; QemuVersion = $qemuVersion
    Acceleration = 'tcg,thread=multi'; MachineType = 'q35'; Artifacts = $artifactSet
})
Write-JsonFile (Join-Path $evidence 'validation-context.json') ([ordered]@{
    EvidenceId = $evidenceId; Mode = $Mode; RunCount = $RunCount
    TimeoutSeconds = $TimeoutSeconds; NoProgressSeconds = $NoProgressSeconds
    QemuPath = $qemu; QemuVersion = $qemuVersion
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    $boundary = if ($Mode -eq 'Positive') { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-string-l1-1-0.dll!strcmp' } else { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-runtime-l1-1-0.dll!_initterm' }
    $summaryObserved = $false; $boundaryObserved = $false; $captured = $false; $timeoutReason = ''
    $deadline = $startUtc.AddSeconds($TimeoutSeconds); $lastProgress = $startUtc; $lastLength = 0
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        $text = Read-Serial $serial
        if ($text.Length -ne $lastLength) {
            $lastLength = $text.Length; $lastProgress = (Get-Date).ToUniversalTime()
            Write-Event $events 'guest-progress' @{ RunId = $runId; Length = $lastLength; LastMarker = (Last-Marker $text) }
        }
        $boundaryObserved = $text.Contains($boundary)
        $summaryObserved = $text.Contains('GXOS_NET10:QPC_REGRESSIONS=') -and
                           $text.Contains('GXOS_NET10:ALLOCATION_CONTEXT_VALID=0')
        if ($boundaryObserved -and $summaryObserved) {
            $captured = $true
            Write-Event $events 'complete-evidence-captured' @{ RunId = $runId; SerialLength = $text.Length }
            Start-Sleep -Milliseconds 500
            break
        }
        try { if ($process.HasExited) { $timeoutReason = 'qemu-exited-before-complete-evidence'; break } } catch { }
        if ((Get-Date).ToUniversalTime() -gt $lastProgress.AddSeconds($NoProgressSeconds)) {
            $timeoutReason = 'guest-no-progress'
            Write-Event $events 'no-progress-timeout' @{ RunId = $runId; Reason = $timeoutReason }
            break
        }
    ...
    if ($alive) {
        Write-Event $events 'qemu-stop-requested' @{ RunId = $runId; Pid = $process.Id; Reason = if ($captured) { 'capture-complete' } else { 'incomplete-evidence' } }
    ...
    $serialItem = if (Test-Path -LiteralPath $serial) { Get-Item -LiteralPath $serial } else { $null }
    $exitCode = $null; try { $process.Refresh(); if ($process.HasExited) { $exitCode = [int]$process.ExitCode } } catch { }
    $endUtc = (Get-Date).ToUniversalTime()
    $runData = [ordered]@{
        EvidenceId = $evidenceId; RunId = $runId; Sequence = $sequence; Mode = $Mode
        Pass = $captured -and $cleanupComplete; QemuPid = $process.Id; QemuVersion = $qemuVersion
        ...
        BoundaryObserved = $boundaryObserved; SummaryObserved = $summaryObserved
        LastObservedGuestMarker = (Last-Marker $finalText)
        FinalSerialLength = if ($null -eq $serialItem) { 0 } else { [int64]$serialItem.Length }
        SerialLog = $serial.Substring($evidence.Length).TrimStart('\','/')
        QemuStdoutLog = $stdout.Substring($evidence.Length).TrimStart('\','/')
        QemuStderrLog = $stderr.Substring($evidence.Length).TrimStart('\','/')
        HarnessEventLog = $events.Substring($evidence.Length).TrimStart('\','/')
        VarsPath = $vars.Substring($evidence.Length).TrimStart('\','/')
        ArtifactSetAfterRun = @(Assert-ArtifactSet $artifactSet)
    }
...
Write-Output "CRT_INITTERM_EVIDENCE_ROOT=$evidence"
Write-Output 'CRT_INITTERM_FINAL_VALIDATION=PASSED'
