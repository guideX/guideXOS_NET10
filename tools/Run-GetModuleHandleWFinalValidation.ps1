...
Write-Json (Join-Path $evidence 'artifact-manifest.json') ([ordered]@{
    ...
    GateDirectory=$gate; Mode=$Mode; ExpectedBoundary=if ($Mode -eq 'Positive') { 'KERNEL32.dll!GetProcAddress' } else { 'KERNEL32.dll!GetModuleHandleW' }
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    while ((Get-Date).ToUniversalTime() -lt $deadline) {
        ...
        $complete = if ($Mode -eq 'Disabled') {
            $summary -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetModuleHandleW')
        } else {
            $summary -and $text.Contains('GXOS_NET10:GETMODULEHANDLEW_CALLER_CONSUMPTION_COMPLETE') -and $text.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcAddress')
        }
    ...
    $finalComplete = if ($Mode -eq 'Disabled') { $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetModuleHandleW') } else { $final.Contains('GXOS_NET10:GETMODULEHANDLEW_CALLER_CONSUMPTION_COMPLETE') -and $final.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:KERNEL32.dll!GetProcAddress') }
...
Write-Output "GETMODULEHANDLEW_EVIDENCE_ROOT=$evidence"
Write-Output 'GETMODULEHANDLEW_FINAL_VALIDATION=PASSED'
