...
try {
    $resourceSha256 = ([BitConverter]::ToString(
        $resourceHasher.ComputeHash($resourceBytes))).Replace('-', '')
} finally {
...
$required = if ($CapacityControl) {
    ...
    @('GXOS_NET10:MANAGED_KERNEL_PHASE46_MODE_SELECTED',
      ...
      'GXOS_NET10:MANAGED_HTTPS_PHASE46_FIXED_SCROLL_PROOF_PASS',
      'GXOS_NET10:MANAGED_HTTPS_PHASE46_NESTED_OPACITY_PROOF_PASS',
