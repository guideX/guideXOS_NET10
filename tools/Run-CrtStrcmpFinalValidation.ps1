...
$validatorScript = Join-Path $root 'tools\Validate-CrtStrcmpEvidence.ps1'
...
for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
    ...
    $boundary = if ($Mode -eq 'Positive') { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-string-l1-1-0.dll!strlen' } else { 'GXOS_NET10:UNEXPECTED_IMPORT_CALL:api-ms-win-crt-string-l1-1-0.dll!strcmp' }
...
Write-Output "CRT_STRCMP_EVIDENCE_ROOT=$evidence"
Write-Output 'CRT_STRCMP_FINAL_VALIDATION=PASSED'
