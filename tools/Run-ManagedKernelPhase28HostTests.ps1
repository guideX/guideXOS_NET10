...
$out = Join-Path $root 'artifacts\managed-kernel-phase28-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase28HostTests\ManagedKernelPhase28HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 28 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 28 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase28HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 28 host tests failed: $LASTEXITCODE" }
