...
$out = Join-Path $root 'artifacts\managed-kernel-phase26-host-tests'
$project = Join-Path $root 'src\ManagedKernelPhase26HostTests\ManagedKernelPhase26HostTests.csproj'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 26 host-test restore failed: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 26 host-test build failed: $LASTEXITCODE" }
} finally {
    Pop-Location
}
$assembly = Join-Path $out 'bin\ManagedKernelPhase26HostTests.dll'
...
if ($LASTEXITCODE -ne 0) { throw "Phase 26 host tests failed: $LASTEXITCODE" }
