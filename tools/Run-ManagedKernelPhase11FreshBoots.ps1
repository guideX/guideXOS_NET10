...
param(
    ...
    [switch]$EnablePhase40Protocol,
    [switch]$EnablePhase41Protocol,
    [switch]$EnablePhase42Protocol,
    [switch]$EnablePhase43Protocol,
    [switch]$EnablePhase43CapacityControl,
    [switch]$EnablePhase44Protocol,
    [switch]$EnablePhase44CapacityControl,
    [switch]$EnablePhase45Protocol,
    ...
    [switch]$EnablePhase51ResetReuseControl,
    [switch]$EnablePhase52Protocol,
    [switch]$EnablePhase52BadPngCrcControl,
    [switch]$EnablePhase53Protocol,
    [switch]$EnablePhase53BadPngCrcControl,
...
if ($EnablePhase40Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase40Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase41Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase41Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase42Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase42Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase43Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase43Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase43CapacityControl -and -not $EnablePhase43Protocol) {
    throw '-EnablePhase43CapacityControl requires -EnablePhase43Protocol.'
}
if ($EnablePhase44Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase44Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase44CapacityControl -and -not $EnablePhase44Protocol) {
    throw '-EnablePhase44CapacityControl requires -EnablePhase44Protocol.'
}
if ($EnablePhase45Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase45Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase45CapacityControl -and -not $EnablePhase45Protocol) {
    throw '-EnablePhase45CapacityControl requires -EnablePhase45Protocol.'
}
if ($EnablePhase46Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase46Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
...
if ($EnablePhase50Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase50Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if (($EnablePhase51Protocol -or $EnablePhase51WrongMimeControl -or $EnablePhase51ResetReuseControl) -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw 'Phase 51 protocol controls require -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase52Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase52Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase52BadPngCrcControl -and -not $EnablePhase52Protocol) {
    throw '-EnablePhase52BadPngCrcControl requires -EnablePhase52Protocol.'
}
if ($EnablePhase52BadPngCrcControl -and $RunCount -ne 1) {
    throw '-EnablePhase52BadPngCrcControl is a single fresh-boot negative control.'
}
if ($EnablePhase53Protocol -and
    (!$EnablePhase15Rx -or $Phase15NetworkBackend -ne 'dgram')) {
    throw '-EnablePhase53Protocol requires -EnablePhase15Rx -Phase15NetworkBackend dgram.'
}
if ($EnablePhase53BadPngCrcControl -and -not $EnablePhase53Protocol) {
    throw '-EnablePhase53BadPngCrcControl requires -EnablePhase53Protocol.'
}
if ($EnablePhase53BadPngCrcControl -and $RunCount -ne 1) {
    throw '-EnablePhase53BadPngCrcControl is a single fresh-boot negative control.'
}
if ($CaptureQemuScreen -and -not ($EnablePhase49Protocol -or $EnablePhase50Protocol -or $EnablePhase51Protocol -or $EnablePhase51WrongMimeControl -or $EnablePhase51ResetReuseControl -or $EnablePhase52Protocol -or $EnablePhase53Protocol)) {
    throw '-CaptureQemuScreen requires a visible Phase 49/50/51/52/53 control.'
}
...
if ($EnablePhase42MalformedControl -and $EnablePhase43Protocol) {
    throw '-EnablePhase42MalformedControl cannot be combined with -EnablePhase43Protocol.'
}
...
function Request-QemuScreenDump11([Net.Sockets.TcpClient]$monitor,
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if (Test-Path -LiteralPath $path) {
            try {
                $ppm = [IO.File]::ReadAllBytes($path)
                $headerEnd = -1
                $newlines = 0
                for ($index = 0; $index -lt $ppm.Length; ++$index) {
                    if ($ppm[$index] -ne 10) { continue }
                    ++$newlines
                    if ($newlines -eq 3) {
                        $headerEnd = $index + 1
                        break
                ...
                if ($headerEnd -gt 0) {
                    $header = [Text.Encoding]::ASCII.GetString($ppm, 0, $headerEnd)
                    $tokens = @($header -split '\s+' | Where-Object { $_.Length -ne 0 })
                    $width = 0
                    $height = 0
                    $maxValue = 0
                    if ($tokens.Count -eq 4 -and $tokens[0] -eq 'P6' -and
                        [int]::TryParse($tokens[1], [ref]$width) -and
                        [int]::TryParse($tokens[2], [ref]$height) -and
                        [int]::TryParse($tokens[3], [ref]$maxValue) -and
                        $width -gt 0 -and $height -gt 0 -and $maxValue -eq 255) {
                        $expectedLength = [int64]$headerEnd +
                            ([int64]$width * [int64]$height * 3)
                        if ([int64]$ppm.Length -eq $expectedLength) {
                            Write-Timeline11 $script:phase11Timeline 'QEMU_SCREEN_DUMP_READY' "bytes=$($ppm.Length)"
                            return
                        }
                    }
                }
...
function Send-Phase32ServerTcpData11([Net.Sockets.UdpClient]$peerUdp,
    ...
    $frame = New-Ipv4Tcp22 $guestMac $peerMac $hostIp $guestIp `
        (New-TcpSegment22 $serverPort $clientPort $serverSequence $clientSequence $tcpFlags `
...
function New-Phase40GzipBody11 {
    $body = New-Object byte[] 16384
    for ($index = 0; $index -lt $body.Length; ++$index) {
        $body[$index] = [byte](($index * 31 + 7) -band 0xFF)
    }
    $stream = [IO.MemoryStream]::new()
    $gzip = [IO.Compression.GZipStream]::new(
        $stream, [IO.Compression.CompressionLevel]::Optimal, $true)
    try {
...
function New-Phase41GzipBody11 {
    $unit = [byte[]](
        0x47, 0x75, 0x69, 0x64, 0x65, 0x58, 0x4F, 0x53, 0x20, 0x34, 0x31,
        0x0D, 0x0A, 0x52, 0xC3, 0xA9, 0x73, 0x75, 0x6D, 0x20,
        0xCE, 0xBB, 0xCE, 0xB7, 0x20, 0xD0, 0x96, 0x20,
        0xE4, 0xB8, 0xAD, 0x20, 0xE2, 0x98, 0x85, 0x20,
        0xF0, 0x9F, 0x99, 0x82, 0x0A)
    $body = New-Object byte[] ($unit.Length * 256)
    for ($repeat = 0; $repeat -lt 256; ++$repeat) {
        [Array]::Copy($unit, 0, $body, $repeat * $unit.Length, $unit.Length)
    }
    $stream = [IO.MemoryStream]::new()
    $gzip = [IO.Compression.GZipStream]::new(
        $stream, [IO.Compression.CompressionLevel]::Optimal, $true)
...
function New-Phase42GzipBody11 {
    if ($EnablePhase42MalformedControl) {
        $html = '<div data-overlong=' + ('x' * 257) + '>bad</div>'
    } else {
        $html = "<!DOCTYPE html><html><head><title>GuideX &amp; Phase 42</title><style>body{color:red}#home{font-weight:bold}</style><script>if (a < b) { x('&amp;'); }</script></head><body><h1 id='home' class=hero>GuideX OS</h1><p class='intro' data-kind=overview>Bounded HTML streams safely.</p><a href='/guide' title='Read &lt;guide&gt;'>Read the guide</a><img src='/logo.svg' alt='GuideX logo' width=64 height='64'/><form id='contact' action='/send' method=post><input name=email type=email required><button type=submit>Send</button></form><div class='unicode' data-x=one disabled>Ré sum λη Ж 中 ★ 🙂 &lt;ok</div><!-- phase42 comment --><textarea>A &amp; B</textarea><p class=long>" + ('x' * 1200) + "</p></body></html>"
    }
...
function New-Phase44GzipBody11 {
...
function New-Phase45GzipBody11 {
...
function New-Phase50GzipBody11 {
    $compressed = New-Phase49GzipBody11
    $inputStream = [IO.MemoryStream]::new($compressed)
    $gzipInput = [IO.Compression.GZipStream]::new($inputStream, [IO.Compression.CompressionMode]::Decompress)
    try {
        $reader = [IO.StreamReader]::new($gzipInput, [Text.Encoding]::UTF8)
    ...
    $html = $html.Replace('GuideX Phase 49', 'GuideX Phase 50').
...
function Compress-Phase51Bytes11([byte[]]$body) {
    $stream = [IO.MemoryStream]::new()
    $gzip = [IO.Compression.GZipStream]::new($stream, [IO.Compression.CompressionLevel]::Optimal, $true)
    try { $gzip.Write($body, 0, $body.Length) } finally { $gzip.Dispose() }
    try { return ,([byte[]]$stream.ToArray()) } finally { $stream.Dispose() }
...
function New-Phase51IndexBody11 {
    $html = '<html><head><link rel="stylesheet" href="/phase51/a.css"><style>.target{color:green}</style><link rel="stylesheet" href="b.css"></head><body><div id="target" class="target">cafe&#233;</div></body></html>'
    return [Text.Encoding]::UTF8.GetBytes($html)
}

function New-Phase51AGzipBody11 {
    $css = @"
.target { color: red; width: 96px; height: 54px; background-color: #112233; border-width: 2px; border-style: solid; border-color: #00FF00; }
"@
    return Compress-Phase51Bytes11 ([Text.Encoding]::UTF8.GetBytes($css))
}

function New-Phase51BGzipBody11 {
    $css = @"
/* second sheet deliberately wins equal-specificity declarations */ .target { color: blue; width: 180px; height: 74px; padding: 8px; background-color: #123456; border-width: 3px; border-style: solid; border-color: white; }
"@
    return Compress-Phase51Bytes11 ([Text.Encoding]::UTF8.GetBytes($css))
}

function Get-Phase52Crc32Byte11([uint32]$crc, [byte]$value) {
    [uint64]$work = ([uint64]$crc) -bxor ([uint64]$value)
    for ($bit = 0; $bit -lt 8; ++$bit) {
        $work = if (($work -band 1) -ne 0) {
            ([uint64]($work -shr 1)) -bxor [uint64]3988292384
        } else {
            [uint64]($work -shr 1)
        }
        $work = [uint64]($work -band [uint64]4294967295)
    }
    return [uint32]$work
}

function Write-Phase52U32([IO.Stream]$stream, [uint32]$value) {
    $stream.WriteByte([byte](($value -shr 24) -band 0xFF)); $stream.WriteByte([byte](($value -shr 16) -band 0xFF))
    $stream.WriteByte([byte](($value -shr 8) -band 0xFF)); $stream.WriteByte([byte]($value -band 0xFF))
}

function Write-Phase52Chunk([IO.Stream]$stream, [byte[]]$type, [byte[]]$data) {
    Write-Phase52U32 $stream ([uint32]$data.Length)
    $stream.Write($type, 0, $type.Length); if ($data.Length -ne 0) { $stream.Write($data, 0, $data.Length) }
    [uint32]$crc = 4294967295
    foreach ($value in $type) { $crc = Get-Phase52Crc32Byte11 $crc $value }
    foreach ($value in $data) { $crc = Get-Phase52Crc32Byte11 $crc $value }
    [uint32]$finalCrc = [uint32](([int64]$crc) -bxor [int64]4294967295)
    Write-Phase52U32 $stream $finalCrc
}

function New-Phase52PngBody11 {
    $width = 48; $height = 32; $bpp = 4; $rowBytes = $width * $bpp
    $inflated = [Collections.Generic.List[byte]]::new()
    [byte[]]$previous = New-Object byte[] $rowBytes
    for ($y = 0; $y -lt $height; ++$y) {
        [byte[]]$row = New-Object byte[] $rowBytes
        for ($x = 0; $x -lt $width; ++$x) {
            $offset = $x * 4; [byte]$r = 0; [byte]$g = 0; [byte]$b = 0; [byte]$a = 255
            if ($y -lt 8) {
                if ($x -lt 12) { $r = 255 } elseif ($x -lt 24) { $g = 255 } elseif ($x -lt 36) { $b = 255 } else { $r = 255; $g = 255; $b = 255 }
            } elseif ($y -lt 16) {
            } elseif ($y -lt 24) {
        ...
        $filter = $y % 5; $inflated.Add([byte]$filter)
        for ($index = 0; $index -lt $rowBytes; ++$index) {
            $left = if ($index -ge $bpp) { $row[$index - $bpp] } else { 0 }
            $up = $previous[$index]
            $upLeft = if ($index -ge $bpp) { $previous[$index - $bpp] } else { 0 }
            $predictor = switch ($filter) {
                0 { 0 }; 1 { $left }; 2 { $up }; 3 { [math]::Floor(($left + $up) / 2) }
                default {
                    $p = $left + $up - $upLeft; $pa = [math]::Abs($p - $left); $pb = [math]::Abs($p - $up); $pc = [math]::Abs($p - $upLeft)
                    if ($pa -le $pb -and $pa -le $pc) { $left } elseif ($pb -le $pc) { $up } else { $upLeft }
                }
            }
            $inflated.Add([byte](($row[$index] - $predictor) -band 0xFF))
        }
        [Array]::Copy($row, $previous, $rowBytes)
    }
    $compressed = [IO.MemoryStream]::new(); $compressed.WriteByte(0x78); $compressed.WriteByte(0x9C)
    ...
    $ihdr = New-Object byte[] 13; $ihdr[3] = 48; $ihdr[7] = 32; $ihdr[8] = 8; $ihdr[9] = 6
    Write-Phase52Chunk $output ([Text.Encoding]::ASCII.GetBytes('IHDR')) $ihdr
    $split = [math]::Max(1, [math]::Ceiling($zlib.Length / 3))
    for ($offset = 0; $offset -lt $zlib.Length;) {
        $count = [math]::Min($split, $zlib.Length - $offset)
        Write-Phase52Chunk $output ([Text.Encoding]::ASCII.GetBytes('IDAT')) ([byte[]]$zlib[$offset..($offset + $count - 1)])
        $offset += $count
    }
    Write-Phase52Chunk $output ([Text.Encoding]::ASCII.GetBytes('IEND')) ([byte[]]@())
    try {
        [byte[]]$result = $output.ToArray()
        if ($EnablePhase52BadPngCrcControl) {
            # Corrupt only the IEND CRC after the complete bounded fixture has
            # been constructed.  The guest still receives image/png and all
            # preceding PNG bytes, so the control exercises guest CRC failure.
            $result[$result.Length - 1] = $result[$result.Length - 1] -bxor 1
        }
        return ,$result
...
function Invoke-Phase34Hop11([Net.Sockets.UdpClient]$peerUdp,
    ...
    $phase51ResetReuse = [bool]$EnablePhase51ResetReuseControl
    ...
    $phase45Capacity = [bool]$EnablePhase45CapacityControl
    $phase44 = [bool]$EnablePhase44Protocol
    $phase44Capacity = [bool]$EnablePhase44CapacityControl
    $phase43 = [bool]$EnablePhase43Protocol
    $phase43Capacity = [bool]$EnablePhase43CapacityControl
    $phase42 = [bool]$EnablePhase42Protocol
    $phase42Malformed = [bool]$EnablePhase42MalformedControl
    $phase41 = [bool]$EnablePhase41Protocol
    $phase40 = [bool]$EnablePhase40Protocol
    $compressedResource = $resourceProof -or $phase40 -or $phase41 -or $phase42 -or $phase43 -or $phase44 -or $phase45 -or $phase46 -or $phase48 -or $phase49 -or $phase50 -or $phase51 -or $phase52 -or $phase53
    $phaseMarker = if ($phase53) { 'PHASE53' } elseif ($phase52) { 'PHASE52' } elseif ($phase51) { 'PHASE51' } elseif ($phase50) { 'PHASE50' } elseif ($phase49) { 'PHASE49' } elseif ($phase48) { 'PHASE48' } elseif ($phase46) { 'PHASE46' } elseif ($phase45) { 'PHASE45' } elseif ($phase44) { 'PHASE44' } elseif ($phase43) { 'PHASE43' } elseif ($phase42) { 'PHASE42' } elseif ($phase41) { 'PHASE41' } elseif ($phase40) { 'PHASE40' } elseif ($resourceProof) { 'PHASE39' } else { 'PHASE34' }
    ...
    if (-not $phase53 -and -not $phase52 -and -not $phase51 -and -not $phase50 -and -not $phase49 -and -not $phase48 -and -not $phase46 -and -not $phase45 -and -not $phase44 -and -not $phase43 -and -not $phase42 -and -not $phase41) {
    ...
    $observedSyn = Receive-ExpectedPhase22TcpFrame $peerUdp $timeoutSeconds `
        ('Phase 34 hop {0} managed SYN' -f $hop) $peerMac $guestMacLocal `
        $guestIp $hostIp $clientPort $serverPort $clientIsn 0 2 `
        ([byte[]]@()) $true
    ...
    if ($phase53) {
        if ($path -eq '/phase53/index.html') {
            $encodedBody = [Text.Encoding]::UTF8.GetBytes('<html><head><link rel="stylesheet" href="/phase53/css/theme.css"></head><body><h1 id="heading">guideXOS Phase 53</h1><div id="hero" class="hero">guideXOS — café ©</div><img id="content" src="/phase53/images/content.png"></body></html>')
            $contentType = 'text/html; charset=utf-8'; $contentEncoding = ''
        } elseif ($path -eq '/phase53/css/theme.css') {
            $encodedBody = Compress-Phase51Bytes11 ([Text.Encoding]::UTF8.GetBytes('.hero { width: 128px; height: 64px; background-color: #204060; background-image: url("../images/background.png"); background-repeat: no-repeat; border-width: 3px; border-style: solid; border-color: white; }'))
            $contentType = 'text/css; charset=utf-8'; $contentEncoding = "Content-Encoding: gzip`r`n"
        } elseif ($path -eq '/phase53/images/content.png' -or $path -eq '/phase53/images/background.png') {
            $encodedBody = New-Phase52PngBody11
            if ($phase53BadPngCrc -and $path -eq '/phase53/images/background.png') {
                $encodedBody[$encodedBody.Length - 1] = $encodedBody[$encodedBody.Length - 1] -bxor 1
            }
            $contentType = 'image/png'; $contentEncoding = ''
        } else {
            throw "Unexpected Phase 53 path: $path"
        }
        $headerBytes = [Text.Encoding]::ASCII.GetBytes(("HTTP/1.1 200 OK`r`nContent-Length: {0}`r`nContent-Type: {1}`r`n{2}Connection: close`r`n`r`n" -f $encodedBody.Length, $contentType, $contentEncoding))
        $responseBytes = New-Object byte[] ($headerBytes.Length + $encodedBody.Length)
        $headerBytes.CopyTo($responseBytes, 0); $encodedBody.CopyTo($responseBytes, $headerBytes.Length)
    } elseif ($phase52) {
        if ($path -eq '/phase52/index.html') {
            $encodedBody = [Text.Encoding]::UTF8.GetBytes('<html><head><link rel="stylesheet" href="/phase52/a.css"><style>.target{color:green}</style><link rel="stylesheet" href="b.css"></head><body><div id="target" class="target">guideXOS Managed Kernel Phase 52 caf&#233; &#8212; r&#233;sum&#233; &#169;</div><img id="hero" class="hero" src="/phase52/image.png"></body></html>')
            $contentType = 'text/html; charset=utf-8'; $contentEncoding = ''
        } elseif ($path -eq '/phase52/image.png') {
            $encodedBody = New-Phase52PngBody11
            $contentType = 'image/png'; $contentEncoding = ''
        } elseif ($path -eq '/phase52/a.css') {
            $encodedBody = Compress-Phase51Bytes11 ([Text.Encoding]::UTF8.GetBytes('.target { color: red; width: 96px; height: 54px; background-color: #112233; border-width: 2px; border-style: solid; border-color: #00FF00; } .hero { width: 96px; height: 64px; background-color: #204060; }'))
            $contentType = 'text/css; charset=utf-8'; $contentEncoding = "Content-Encoding: gzip`r`n"
        } else {
            $encodedBody = Compress-Phase51Bytes11 ([Text.Encoding]::UTF8.GetBytes('.target { color: blue; width: 180px; height: 74px; padding: 8px; background-color: #123456; border-width: 3px; border-style: solid; border-color: white; }'))
            $contentType = 'text/css; charset=utf-8'; $contentEncoding = "Content-Encoding: gzip`r`n"
        }
        $headerBytes = [Text.Encoding]::ASCII.GetBytes(("HTTP/1.1 200 OK`r`nContent-Length: {0}`r`nContent-Type: {1}`r`n{2}Connection: close`r`n`r`n" -f $encodedBody.Length, $contentType, $contentEncoding))
        $responseBytes = New-Object byte[] ($headerBytes.Length + $encodedBody.Length)
        ...
        $encodedBody = if ($phase51ResetReuse) { if ($hop -eq 0 -or $hop -eq 2) { New-Phase51IndexBody11 } elseif ($hop -eq 1 -or $hop -eq 3) { New-Phase51AGzipBody11 } else { New-Phase51BGzipBody11 } } elseif ($hop -eq 0) { New-Phase51IndexBody11 } elseif ($hop -eq 1) { New-Phase51AGzipBody11 } else { New-Phase51BGzipBody11 }
        $headerBytes = [Text.Encoding]::ASCII.GetBytes(
            ("HTTP/1.1 200 OK`r`nContent-Length: {0}`r`nContent-Type: {1}`r`n{2}Connection: close`r`n`r`n" -f
                $encodedBody.Length,
                $(if ($hop -eq 0 -or ($phase51ResetReuse -and $hop -eq 2)) { 'text/html; charset=utf-8' } elseif ($hop -eq 1 -and ($EnablePhase51WrongMimeControl -or $phase51ResetReuse)) { 'text/html; charset=utf-8' } else { 'text/css; charset=utf-8' }),
                $(if ($hop -eq 0 -or ($phase51ResetReuse -and $hop -eq 2)) { '' } else { "Content-Encoding: gzip`r`n" })))
        $responseBytes = New-Object byte[] ($headerBytes.Length + $encodedBody.Length)
        $headerBytes.CopyTo($responseBytes, 0)
        $encodedBody.CopyTo($responseBytes, $headerBytes.Length)
    } elseif ($compressedResource) {
        $encodedBody = if ($phase50) { New-Phase50GzipBody11 } elseif ($phase49) { New-Phase49GzipBody11 } elseif ($phase48) { New-Phase48GzipBody11 } elseif ($phase46) { New-Phase46GzipBody11 } elseif ($phase45) { New-Phase45GzipBody11 } elseif ($phase44) { New-Phase44GzipBody11 } elseif ($phase43) { New-Phase43GzipBody11 } elseif ($phase42) { New-Phase42GzipBody11 } elseif ($phase41) { New-Phase41GzipBody11 } elseif ($phase40) { New-Phase40GzipBody11 } else {
        ...
        $headerBytes = [Text.Encoding]::ASCII.GetBytes(
            ("HTTP/1.1 200 OK`r`nContent-Length: {0}`r`nContent-Type: {1}`r`n{2}Connection: close`r`n`r`n" -f
                $encodedBody.Length,
                $(if ($phase50 -or $phase49 -or $phase48 -or $phase46 -or $phase45 -or $phase44 -or $phase43 -or $phase42) { 'text/html; charset=utf-8' } elseif ($phase41) { 'text/plain; charset=utf-8' } else { 'application/octet-stream' }),
                $(if ($phase40 -or $phase41 -or $phase42 -or $phase43 -or $phase44 -or $phase45 -or $phase46 -or $phase48 -or $phase49 -or $phase50) { "Content-Encoding: gzip`r`n" } else { '' })))
        $responseBytes = New-Object byte[] ($headerBytes.Length + $encodedBody.Length)
        $headerBytes.CopyTo($responseBytes, 0)
        $encodedBody.CopyTo($responseBytes, $headerBytes.Length)
    ...
    for ($offset = 0; $offset -lt $responseBytes.Length;) {
        ...
        Send-Phase32ServerTcpData11 $peerUdp $rxPort $injectionLog $peerMac $guestMacLocal `
            ...
            (0x6200 + $hop * 0x100 + $responseChunk) $timeoutSeconds (-not ($phase51 -or $phase52 -or $phase53)) `
            $(if (($phase51 -or $phase52 -or $phase53) -and $offset + $count -eq $responseBytes.Length) { 0x19 } else { 0x18 })
        [uint32]$serverNext = $serverNext + $record.Length
        if (($phase51 -or $phase52 -or $phase53) -and $offset + $count -eq $responseBytes.Length) {
            [uint32]$serverNext = $serverNext + 1
        }
        if ($resourceProof) {
            $bodyProgress = [Math]::Min($(if ($phase45) { 1024 } elseif ($phase43) { 2566 } elseif ($phase42) { 1894 } elseif ($phase41) { 10496 } else { 16884 }),
    ...
    if ($phase51 -or $phase52 -or $phase53) {
        [uint32]$earlyFinNext = $clientNext + 1
        $earlyFinalAck = New-TcpSegment22 $serverPort $clientPort $serverNext $earlyFinNext 0x10 `
            $hostIp $guestIp ([byte[]]@()) $false
        $earlyFinalAckFrame = New-Ipv4Tcp22 $guestMacLocal $peerMac $hostIp $guestIp $earlyFinalAck (0x62FF + $hop)
        Require11 ($peerUdp.Send($earlyFinalAckFrame, $earlyFinalAckFrame.Length,
            '127.0.0.1', $rxPort) -eq $earlyFinalAckFrame.Length) `
            "Phase 51 early final ACK send was short for hop $hop."
        Write-Phase22Frame $injectionLog ('phase51_early_final_ack_{0}' -f $hop) $earlyFinalAckFrame
    }
    if ($phase43Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE43_CAPACITY_NEGATIVE_PASS' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE43'
    }
    if ($phase42Malformed) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE42_TOKENIZER_FAILURE=0x0000000000000005' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE42'
    }
    if ($phase51ResetReuse -and $hop -eq 1) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_RESET_REUSE_WRONG_MIME_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase51 -and $EnablePhase51WrongMimeControl -and $hop -eq 1) {
    ...
    if ($phase53 -and $path -eq '/phase53/images/background.png') {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_REFERENCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_FETCH_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        if ($phase53BadPngCrc) {
            Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_FAILURE' $deadline $process $stream $serialLog $text $receiveBuffer
            Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE53_START_FAILED' $deadline $process $stream $serialLog $text $receiveBuffer
            return 'PASS_PHASE53_BAD_PNG_CRC'
        }
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_DECODE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_BACKGROUND_PAINT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_VISIBLE_CSS_IMAGE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE53_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'PASS_PHASE53'
    }
    if ($phase52 -and -not $phase52BadPngCrc -and $path -eq '/phase52/image.png') {
        # The terminal image body has been authenticated and delivered.  The
        # guest publishes the visual proof before the ordinary close exchange,
        # so observe that proof here and let the outer runner stop this owned
        # VM instead of waiting for a network event after the terminal result.
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_DECODE_PASS' $deadline `
            $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_IMAGE_RASTER_PASS' $deadline `
            $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_GOP_PRESENT_PASS' $deadline `
            $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_VISIBLE_IMAGE_PASS' $deadline `
            $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE52_PASS' $deadline `
            $process $stream $serialLog $text $receiveBuffer
        return 'PASS_PHASE52'
    }

    if ($phase51 -or $phase52 -or $phase53) {
        # The final response segment carries the peer FIN.  The compact fixture
        # therefore presents the authenticated body and EOF in one RX boundary.
        [uint32]$finNext = $clientNext + 1
        [uint32]$peerFinNext = $serverNext
        $managedAck = Receive-ExpectedPhase22TcpFrame $peerUdp $timeoutSeconds `
            ('Phase 51/52/53 hop {0} managed FIN ACK' -f $hop) $peerMac $guestMacLocal $guestIp $hostIp `
            $clientPort $serverPort $clientNext $peerFinNext 0x10 ([byte[]]@()) $false
        Write-Phase22Frame $injectionLog ('phase51_managed_fin_ack_{0}' -f $hop) $managedAck
        $managedFin = Receive-ExpectedPhase22TcpFrame $peerUdp $timeoutSeconds `
            ('Phase 51 hop {0} managed FIN' -f $hop) $peerMac $guestMacLocal $guestIp $hostIp `
            $clientPort $serverPort $clientNext $peerFinNext 0x11 ([byte[]]@()) $false
        Write-Phase22Frame $injectionLog ('phase51_managed_fin_{0}' -f $hop) $managedFin
        ...
        for ($finalAckRetry = 0; $finalAckRetry -lt 12; ++$finalAckRetry) {
            if ($finalAckRetry -ne 0) { Start-Sleep -Milliseconds 100 }
            Require11 ($peerUdp.Send($finalAckFrame, $finalAckFrame.Length,
                '127.0.0.1', $rxPort) -eq $finalAckFrame.Length) `
                "Phase 51 final ACK send was short for hop $hop."
            Write-Phase22Frame $injectionLog ('phase51_final_ack_{0}_{1}' -f $hop, $finalAckRetry) $finalAckFrame
        }
    } else {
    ...
    if ($phase46Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_CAPACITY_NEGATIVE_PASS' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE46'
    }
    if ($phase45Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_CAPACITY_NEGATIVE_PASS' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE45'
    }
    if ($phase44Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE44_CSS_CAPACITY_NEGATIVE_PASS' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE44'
    }
    return 'PASS'
}
...
function Invoke-Phase34HttpsExchange11([Net.Sockets.UdpClient]$peerUdp,
    ...
    $phase40 = [bool]$EnablePhase40Protocol
    $compressedResource = $resourceProof -or $phase40 -or $phase41 -or $phase42 -or $phase43 -or $phase44 -or $phase45 -or $phase46 -or $phase48 -or $phase49 -or $phase50 -or $phase51 -or $phase52 -or $phase53
    $phaseMarker = if ($phase53) { 'PHASE53' } elseif ($phase52) { 'PHASE52' } elseif ($phase51) { 'PHASE51' } elseif ($phase50) { 'PHASE50' } elseif ($phase49) { 'PHASE49' } elseif ($phase48) { 'PHASE48' } elseif ($phase46) { 'PHASE46' } elseif ($phase45) { 'PHASE45' } elseif ($phase44) { 'PHASE44' } elseif ($phase43) { 'PHASE43' } elseif ($phase42) { 'PHASE42' } elseif ($phase41) { 'PHASE41' } elseif ($phase40) { 'PHASE40' } else { 'PHASE34' }
    Wait-Marker11 'GXOS_NET10:MANAGED_DHCP_DISCOVER_SENT' $deadline $process $stream $serialLog $text $receiveBuffer
    ...
    if ($phase42) {
        Start-Sleep -Milliseconds 25
        Require11 ($peerUdp.Send($offerFrame, $offerFrame.Length,'127.0.0.1',$rxPort) -eq $offerFrame.Length) 'Phase 42 DHCPOFFER retry send was short.'
        Write-Phase20Frame $injectionLog 'phase42_dhcpoffer_retry' $offerFrame
    }
    Wait-Marker11 'GXOS_NET10:MANAGED_DHCP_REQUEST_SENT' $deadline $process $stream $serialLog $text $receiveBuffer
    ...
    Wait-Marker11 'GXOS_NET10:MANAGED_DHCP_BOUND' $deadline $process $stream $serialLog $text $receiveBuffer
    $configuredMarker = $phaseMarker
    Wait-Marker11 ("GXOS_NET10:MANAGED_HTTPS_{0}_CONFIGURED" -f $configuredMarker) $deadline $process $stream $serialLog $text $receiveBuffer
    $zeroMac = [byte[]](0,0,0,0,0,0)
    $arpRequest = New-Phase16ArpFrame11 $broadcastMac $guestMacBytes 1 $guestIpBytes $zeroMac $hostIpBytes
    ...
    Wait-Marker11 ("GXOS_NET10:MANAGED_HTTPS_{0}_REQUEST_STARTED" -f $phaseMarker) $deadline $process $stream $serialLog $text $receiveBuffer
    ...
    $hopCount = if ($phase53) { 4 } elseif ($phase52) { 4 } elseif ($phase51ResetReuse) { 5 } elseif ($phase51) { 3 } elseif ($compressedResource) { 1 } elseif ($negativeControl) { 2 } else { 4 }
    if ($compressedResource) {
        if ($phase53) { $hosts = @('www.example.com','www.example.com','www.example.com','www.example.com'); $ports = @(443,443,443,443); $paths = @('/phase53/index.html','/phase53/css/theme.css','/phase53/images/content.png','/phase53/images/background.png') }
        elseif ($phase52) { $hosts = @('www.example.com','www.example.com','www.example.com','www.example.com'); $ports = @(443,443,443,443); $paths = @('/phase52/index.html','/phase52/a.css','/phase52/b.css','/phase52/image.png') }
        elseif ($phase51ResetReuse) { $hosts = @('www.example.com','www.example.com','www.example.com','www.example.com','www.example.com'); $ports = @(443,443,443,443,443); $paths = @('/phase51/index.html','/phase51/a.css','/phase51/index.html','/phase51/a.css','/phase51/b.css') }
        elseif ($phase51) { $hosts = @('www.example.com','www.example.com','www.example.com'); $ports = @(443,443,443); $paths = @('/phase51/index.html','/phase51/a.css','/phase51/b.css') }
        elseif ($phase50) { $hosts = @('www.example.com'); $ports = @(443); $paths = @('/phase50/gzip') }
        elseif ($phase49) { $paths = @('/phase49/gzip') }
        ...
        elseif ($phase42) { $paths = @('/phase42/gzip') }
        elseif ($phase41) { $paths = @('/phase41/gzip') }
        elseif ($phase40) { $paths = @('/phase40/gzip') }
        else { $paths = @('/phase39/resource') }
    ...
    for ($hop = 0; $hop -lt $hopCount; ++$hop) {
        ...
        if ($phase53 -and $hopResult -eq 'PASS_PHASE53') {
            return 'PASS_PHASE53'
        }
        if ($phase52 -and $hopResult -eq 'PASS_PHASE52') {
            return 'PASS_PHASE52'
        }
        ...
        if ($phase46Capacity -and $hopResult -eq 'NEGATIVE_PASS_PHASE46') {
            return 'NEGATIVE_PASS_PHASE46'
        }
        if ($phase45Capacity -and $hopResult -eq 'NEGATIVE_PASS_PHASE45') {
            return 'NEGATIVE_PASS_PHASE45'
        }
        if ($phase44Capacity -and $hopResult -eq 'NEGATIVE_PASS_PHASE44') {
            return 'NEGATIVE_PASS_PHASE44'
        }
        if ($phase43Capacity -and $hopResult -eq 'NEGATIVE_PASS_PHASE43') {
            return 'NEGATIVE_PASS_PHASE43'
        }
        if ($phase42Malformed -and $hopResult -eq 'NEGATIVE_PASS_PHASE42') {
            return 'NEGATIVE_PASS_PHASE42'
        }
        if ($hop -lt $hopCount - 1 -and -not ($phase51 -or $phase52 -or $phase53)) {
            Wait-Marker11 ('GXOS_NET10:MANAGED_HTTPS_PHASE34_REDIRECT_FOLLOWED=0x{0:X16}' -f ($hop + 1)) `
                $deadline $process $stream $serialLog $text $receiveBuffer
        }
    }
    if ($phase46Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_CAPACITY_NEGATIVE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE46'
    }
    if ($phase45Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_CAPACITY_NEGATIVE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE45'
    }
    if ($phase44Capacity) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE44_CSS_CAPACITY_NEGATIVE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'NEGATIVE_PASS_PHASE44'
    }
    if ($phase43Capacity) {
    ...
    if ($phase53BadPngCrc) {
        ...
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_DECODE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_BACKGROUND_PAINT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_GOP_PRESENT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE53_VISIBLE_CSS_IMAGE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        ...
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE53_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'PASS_PHASE53'
    } elseif ($phase52BadPngCrc) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_BAD_PNG_CRC_CONTROL_PASS' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE52_START_FAILED' `
            $deadline $process $stream $serialLog $text $receiveBuffer
        return 'PASS_PHASE52_BAD_PNG_CRC'
    } elseif ($phase52) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_DECODE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_IMAGE_RASTER_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_GOP_PRESENT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE52_VISIBLE_IMAGE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        if ($null -ne $screenMonitor -and -not [string]::IsNullOrEmpty($screenPath)) {
            Request-QemuScreenDump11 $screenMonitor $process $injectionLog $screenPath $deadline
        }
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE52_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        return 'PASS_PHASE52'
    } elseif ($phase51ResetReuse) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_GOP_PRESENT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_VISIBLE_PAGE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        if ($null -ne $screenMonitor -and -not [string]::IsNullOrEmpty($screenPath)) {
            Request-QemuScreenDump11 $screenMonitor $process $injectionLog $screenPath $deadline
        }
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE51_RESET_REUSE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase51) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE51_GOP_PRESENT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        ...
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE51_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase50) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE50_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE50_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE50_GOP_PRESENT_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE50_VISIBLE_PAGE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        if ($null -ne $screenMonitor -and -not [string]::IsNullOrEmpty($screenPath)) {
            Request-QemuScreenDump11 $screenMonitor $process $injectionLog $screenPath $deadline
        }
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE50_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase49) {
        ...
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE45_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE45_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE45_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase44) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE44_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE44_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE44_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase43) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE43_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE43_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE43_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase42) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE42_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE42_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE42_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase41) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE41_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE41_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE41_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } elseif ($phase40) {
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE40_RESOURCE_COMPLETE' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_HTTPS_PHASE40_RESOURCE_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
        Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE40_PASS' $deadline $process $stream $serialLog $text $receiveBuffer
    } else {
    ...
    if ($phase51ResetReuse) {
        return 'PASS_PHASE51_RESET_REUSE'
    } elseif ($phase51) {
        return 'PASS_PHASE51'
    } elseif ($phase50) {
        return 'PASS_PHASE50'
...
Require11 ($RunCount -ge 3 -or $EnablePhase45CapacityControl -or $EnablePhase46CapacityControl -or $EnablePhase51ResetReuseControl -or $EnablePhase52BadPngCrcControl) `
    'Three fresh ManagedKernel Phase 11 boots are required unless running a single capacity negative control or the Phase 51 reset/reuse lifecycle proof.'
    ...
    for ($sequence = 1; $sequence -le $RunCount; $sequence++) {
        ...
        try {
            ...
            if ($EnableManagedKernelPhase35) {
                ...
                if ($Phase15AllowHarnessDeferral) {
                    ...
                    if ($EnablePhase53Protocol -or $EnablePhase52Protocol) {
                        ...
                        if ($EnablePhase53Protocol -and $phase15Outcome -eq 'PASS_PHASE53') {
                            Stop-OwnedQemu11 $process
                            $finalText = $text.ToString()
                            $phase53Markers = @(
                                'GXOS_NET10:MANAGED_KERNEL_PHASE53_MODE_SELECTED',
                                'GXOS_NET10:MANAGED_KERNEL_PHASE53_STARTING',
                            ...
                            foreach ($marker in $phase53Markers) {
                                Require11 $finalText.Contains($marker) "Boot $sequence missing marker: $marker"
                            }
                            ...
                            if ($null -ne $injectionLog) { $injectionLog.Dispose(); $injectionLog = $null }
                            if ($null -ne $logStream) { $logStream.Dispose(); $logStream = $null }
                            if ($null -ne $stream) { $stream.Dispose(); $stream = $null }
                            if ($null -ne $client) { $client.Dispose(); $client = $null }
                            if ($null -ne $peerUdp) { $peerUdp.Dispose(); $peerUdp = $null }
                            if ($null -ne $monitor) { $monitor.Dispose(); $monitor = $null }
                            if ($null -ne $timeline) { $timeline.Dispose(); $timeline = $null }
                        ...
                        if ($EnablePhase53BadPngCrcControl -and $phase15Outcome -eq 'PASS_PHASE53_BAD_PNG_CRC') {
                            ...
                            $badCssMarkers = @(
                                'GXOS_NET10:MANAGED_KERNEL_PHASE53_MODE_SELECTED',
                                'GXOS_NET10:MANAGED_KERNEL_PHASE53_STARTING',
                                'GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_FAILURE',
                                'GXOS_NET10:MANAGED_KERNEL_PHASE53_START_FAILED',
                                'GXOS_NET10:MANAGED_KERNEL_PHASE14_ACCOUNTING_RESTORED')
                            foreach ($marker in $badCssMarkers) {
                                Require11 $finalText.Contains($marker) "Boot $sequence missing marker: $marker"
                            }
                            Require11 (!$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_DECODE_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_BACKGROUND_PAINT_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_GOP_PRESENT_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_VISIBLE_CSS_IMAGE_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE53_PASS') -and
                                !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                                !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                                !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                                "Boot $sequence reported an unexpected Phase 53 bad-CRC result."
                            if ($null -ne $injectionLog) { $injectionLog.Dispose(); $injectionLog = $null }
                            if ($null -ne $logStream) { $logStream.Dispose(); $logStream = $null }
                            if ($null -ne $stream) { $stream.Dispose(); $stream = $null }
                            if ($null -ne $client) { $client.Dispose(); $client = $null }
                        ...
                        if ($CaptureQemuScreen -and $phase15Outcome -eq 'PASS_PHASE52') {
                            Request-QemuScreenDump11 $monitor $process $injectionLog $screenPath $deadline
                        }
                        if ($phase15Outcome -eq 'PASS_PHASE52') {
                            # Phase 52 is terminal for this fresh boot.  Stop
                            # the owned VM here, before the generic Phase 15
                            # bookkeeping can wait for post-proof traffic.
                            Stop-OwnedQemu11 $process
                            $finalText = $text.ToString()
                            $phase52Markers = @(
                            ...
                            foreach ($marker in $phase52Markers) {
                                Require11 $finalText.Contains($marker) "Boot $sequence missing marker: $marker"
                            }
                            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                                !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                            ...
                            Require11 ((Get-FileHash -LiteralPath $payload -Algorithm SHA256).Hash.ToUpperInvariant() -eq $expectedHash) `
                                "ManagedKernel payload hash changed on boot $sequence."
                            Require11 (([regex]::Matches($finalText, 'GXOS_NET10:MANAGED_KERNEL_PHASE11_PASS')).Count -eq 1) `
                                'Phase 11 pass marker count was not one.'
                            if ($null -ne $injectionLog) { $injectionLog.Dispose(); $injectionLog = $null }
                            if ($null -ne $logStream) { $logStream.Dispose(); $logStream = $null }
                            if ($null -ne $stream) { $stream.Dispose(); $stream = $null }
                            if ($null -ne $client) { $client.Dispose(); $client = $null }
                        ...
                        if ($phase15Outcome -eq 'PASS_PHASE52_BAD_PNG_CRC') {
                            Wait-Marker11 'GXOS_NET10:MANAGED_KERNEL_PHASE14_ACCOUNTING_RESTORED' `
                                $deadline $process $stream $logStream $text $buffer
                            Stop-OwnedQemu11 $process
                            $finalText = $text.ToString()
                            $badCrcMarkers = @(
                                ...
                                'GXOS_NET10:MANAGED_KERNEL_PHASE14_ACCOUNTING_RESTORED')
                            foreach ($marker in $badCrcMarkers) {
                                Require11 $finalText.Contains($marker) "Boot $sequence missing marker: $marker"
                            }
                            Require11 (!$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_DECODE_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_IMAGE_RASTER_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_GOP_PRESENT_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_VISIBLE_IMAGE_PASS') -and
                                !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_PASS') -and
                                !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                                !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                                !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                                "Boot $sequence reported an unexpected bad-CRC result."
                            Require11 ((Get-FileHash -LiteralPath $payload -Algorithm SHA256).Hash.ToUpperInvariant() -eq $expectedHash) `
                                "ManagedKernel payload hash changed on boot $sequence."
                            Require11 (([regex]::Matches($finalText, 'GXOS_NET10:MANAGED_KERNEL_PHASE11_PASS')).Count -eq 1) `
                                'Phase 11 pass marker count was not one.'
                    ...
                    } elseif ($EnablePhase51Protocol -or $EnablePhase51WrongMimeControl -or $EnablePhase51ResetReuseControl) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                        ...
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            ...
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                            $false $false
                    } elseif ($EnablePhase44Protocol -or $EnablePhase44CapacityControl) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                            $false $false
                    } elseif ($EnablePhase43Protocol) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                            $false $false
                    } elseif ($EnablePhase42Protocol) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                            $false $false
                    } elseif ($EnablePhase41Protocol) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                            $false $false
                    } elseif ($EnablePhase40Protocol) {
                        $phase15Outcome = Invoke-Phase34HttpsExchange11 `
                            $peerUdp $rxPort $TimeoutSeconds $process $stream `
                            $logStream $text $buffer $injectionLog `
                            $guestMacBytes $hostMacBytes $guestIpBytes $hostIpBytes `
                    ...
                    if ($EnablePhase53Protocol -or $EnablePhase53BadPngCrcControl -or $EnablePhase52Protocol -or $EnablePhase52BadPngCrcControl) {
                        # Phase 52 completes its terminal resource proof in the
                        # HTTPS helper above and has no later Phase 15 traffic.
                    } elseif ($EnablePhase51Protocol -or $EnablePhase51WrongMimeControl -or $EnablePhase51ResetReuseControl) {
                        # Phase 51 completed its multi-resource page proof and
                        # screen capture in the resource helper above.
                    } elseif ($EnablePhase50Protocol) {
                        # Phase 50 completed its bounded Unicode visible-page
                        # proof and screen capture in the resource helper.
                    } elseif ($EnablePhase49Protocol) {
                        ...
                        # Phase 45 completed its bounded layout exchange and
                        # waits for its proof markers above.
                    } elseif ($EnablePhase44Protocol -or $EnablePhase44CapacityControl) {
                        # Phase 44 completed its bounded gzip HTML/CSS cascade
                        # exchange and waits for its proof markers above.
                    } elseif ($EnablePhase43Protocol) {
                        # Phase 43 completed its bounded gzip HTML/document
                        # exchange and waits for its proof markers above.
                    } elseif ($EnablePhase42Protocol) {
                        # Phase 42 completed its bounded gzip HTML/tokenizer
                        # exchange and waits for its proof markers above.
                    } elseif ($EnablePhase41Protocol) {
                        # Phase 41 completed its single gzip text resource
                        # exchange and waits for its proof markers above.
                    } elseif ($EnablePhase40Protocol) {
                        # Phase 40 completed its single gzip resource exchange
                        # and waits for its proof markers in the helper above.
                    } elseif ($EnablePhase39Protocol) {
            ...
            if ($EnablePhase51WrongMimeControl) {
                ...
                Stop-OwnedQemu11 $process
                Pump-Serial11 $stream $logStream $text $buffer
            } elseif ($EnablePhase53Protocol -or $EnablePhase52Protocol) {
                # Phase 52 is a terminal visual proof and intentionally leaves
                # the guest alive after publishing its result.  Stop only this
                # run's owned QEMU so the serial transcript can be drained and
                # the fresh-boot harness can advance to the next run.
                Stop-OwnedQemu11 $process
            } else {
                Pump-Serial11 $stream $logStream $text $buffer
            }
        ...
        if ($EnablePhase34NegativeControl) {
            ...
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        ...
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
        } elseif ($EnablePhase46CapacityControl) {
            Require11 ($phase15Outcome -eq 'NEGATIVE_PASS_PHASE46') `
                "Boot $sequence did not complete the Phase 46 paint capacity negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE46_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_CAPACITY_CONTROL_VALIDATED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_CAPACITY_NEGATIVE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE14_BLOCKED=DRIVER_START') -and
                        $finalText.Contains('GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE46_RESOURCE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE46_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 46 paint capacity-control result."
        } elseif ($EnablePhase53BadPngCrcControl) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE53_BAD_PNG_CRC') `
                "Boot $sequence did not complete the Phase 53 CSS-image bad-CRC negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE53_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_FAILURE') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_DECODE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_BACKGROUND_PAINT_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_VISIBLE_CSS_IMAGE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE53_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 53 CSS-image bad-CRC result."
        } elseif ($EnablePhase53Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE53') `
                "Boot $sequence did not complete the Phase 53 CSS-image proof: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE53_MODE_SELECTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE53_STARTING') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_REFERENCE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_FETCH_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_CSS_IMAGE_DECODE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_BACKGROUND_PAINT_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_GOP_PRESENT_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE53_VISIBLE_CSS_IMAGE_PASS') -and
                        ...
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 53 CSS-image result."
        } elseif ($EnablePhase52BadPngCrcControl) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE52_BAD_PNG_CRC') `
                "Boot $sequence did not complete the Phase 52 bad-CRC negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_MODE_SELECTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_STARTING') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_BAD_PNG_CRC_REASON=0x0000000000000005') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_BAD_PNG_CRC_IMAGE_SLOT_COUNT=0x0000000000000000') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_BAD_PNG_CRC_CONTROL_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE14_ACCOUNTING_RESTORED') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_DECODE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_IMAGE_RASTER_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_GOP_PRESENT_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_VISIBLE_IMAGE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_PASS') -and
                        ...
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 52 bad-CRC result."
        } elseif ($EnablePhase52Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE52') `
                "Boot $sequence did not complete the Phase 52 external PNG image proof: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_MODE_SELECTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE52_STARTING') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_HEADER_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_PNG_DECODE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_IMAGE_RASTER_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_GOP_PRESENT_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE52_VISIBLE_IMAGE_PASS') -and
                        ...
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 52 external PNG image result."
            if ($CaptureQemuScreen) {
                Require11 (Test-Path -LiteralPath $screenPath) `
                    "Boot $sequence did not produce the requested QEMU screen capture."
            }
        } elseif ($EnablePhase51ResetReuseControl) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE51_RESET_REUSE') "Boot $sequence did not complete the Phase 51 reset/reuse proof: $phase15Outcome."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE51_RESET_REUSE_MODE_SELECTED') "Boot $sequence missed the reset/reuse mode marker."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE51_RESET_REUSE_STARTING') "Boot $sequence missed the reset/reuse start marker."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_RESET_STATE_ASSERTIONS=PASS') "Boot $sequence missed reset-state assertions."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_RESET_REUSE_WRONG_MIME_PASS') "Boot $sequence missed the wrong-MIME reset boundary."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_RESOURCE_PASS') "Boot $sequence missed the valid-page resource pass after reset."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_GOP_PRESENT_PASS') "Boot $sequence missed GOP presentation after reset."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_VISIBLE_PAGE_PASS') "Boot $sequence missed visible-page proof after reset."
            Require11 $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE51_RESET_REUSE_PASS') "Boot $sequence missed the reset/reuse kernel pass."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported a machine fault during reset/reuse."
            ...
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE51_MODE_SELECTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE51_STARTING') -and
                        [regex]::IsMatch($finalText, 'GXOS_NET10:MANAGED_HTTPS_PHASE51_EXTERNAL_LOADED=0x0*2(?:\r?\n|$)') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_STYLESHEET_A_CONTENT_ENCODING=0x') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_STYLESHEET_B_CONTENT_ENCODING=0x') -and
                        [regex]::IsMatch($finalText, 'GXOS_NET10:MANAGED_HTTPS_PHASE51_TARGET_COLOR=0x0*FF0000FF(?:\r?\n|$)') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_GOP_PRESENT_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE51_VISIBLE_PAGE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 51 external stylesheet result."
            if ($CaptureQemuScreen) {
                Require11 (Test-Path -LiteralPath $screenPath) `
                    "Boot $sequence did not produce the requested QEMU screen capture."
            }
        } elseif ($EnablePhase50Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE50') `
                "Boot $sequence did not complete the Phase 50 bounded Unicode visible framebuffer proof: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_FRAMEBUFFER_DESCRIPTOR_ACCEPTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE50_MODE_SELECTED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE50_STARTING') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE50_FONT_COVERAGE_FLAGS=') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE50_FONT_LATIN1_LOOKUPS=') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE50_FONT_NBSP_COUNT=') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE50_GOP_PRESENT_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE50_VISIBLE_PAGE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 50 bounded Unicode result."
            if ($CaptureQemuScreen) {
                Require11 (Test-Path -LiteralPath $screenPath) `
                    "Boot $sequence did not produce the requested QEMU screen capture."
            }
            ...
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_VERIFIED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE46_PAINT_HASH_WORD=') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 46 display-list result."
        } elseif ($EnablePhase45CapacityControl) {
            Require11 ($phase15Outcome -eq 'NEGATIVE_PASS_PHASE45') `
                "Boot $sequence did not complete the Phase 45 layout capacity negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE45_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_CAPACITY_CONTROL_VALIDATED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_CAPACITY_NEGATIVE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE14_BLOCKED=DRIVER_START') -and
                        $finalText.Contains('GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE45_RESOURCE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE45_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 45 layout capacity-control result."
        } elseif ($EnablePhase45Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE45') `
                "Boot $sequence did not complete the Phase 45 bounded layout proof: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_VERIFIED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE45_LAYOUT_HASH_WORD=') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 45 layout result."
        } elseif ($EnablePhase44CapacityControl) {
            Require11 ($phase15Outcome -eq 'NEGATIVE_PASS_PHASE44') `
                "Boot $sequence did not complete the Phase 44 CSS capacity negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE44_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE44_CSS_CAPACITY_CONTROL_VALIDATED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE44_CSS_CAPACITY_NEGATIVE_PASS') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE14_BLOCKED=DRIVER_START') -and
                        $finalText.Contains('GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE44_RESOURCE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE44_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 44 CSS capacity-control result."
        } elseif ($EnablePhase44Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE44') `
                "Boot $sequence did not complete the Phase 44 bounded CSS cascade proof: $phase15Outcome."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
        } elseif ($EnablePhase43CapacityControl) {
            Require11 ($phase15Outcome -eq 'NEGATIVE_PASS_PHASE43') `
                "Boot $sequence did not complete the Phase 43 node-capacity negative control: $phase15Outcome."
            Require11 ($finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE43_START_FAILED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE43_CAPACITY_CONTROL_VALIDATED') -and
                        $finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE14_BLOCKED=DRIVER_START') -and
                        $finalText.Contains('GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE43_RESOURCE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE43_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected Phase 43 capacity-control result."
        } elseif ($EnablePhase43Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE43') `
                "Boot $sequence did not complete the Phase 43 bounded HTML document proof: $phase15Outcome."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
        } elseif ($EnablePhase42MalformedControl) {
            Require11 ($phase15Outcome -eq 'NEGATIVE_PASS_PHASE42') `
                "Boot $sequence did not reject the malformed Phase 42 HTML: $phase15Outcome."
            Require11 ($finalText.Contains(
                'GXOS_NET10:MANAGED_KERNEL_PHASE42_START_FAILED')) `
                "Boot $sequence did not report the malformed Phase 42 start failure."
            Require11 ($finalText.Contains(
                'GXOS_NET10:MANAGED_KERNEL_PHASE14_BLOCKED=DRIVER_START') -and
                        $finalText.Contains(
                'GXOS_NET10:FAIL:managed-kernel-phase14-driver-proof')) `
                "Boot $sequence did not reach the expected kernel health failure boundary."
            Require11 (!$finalText.Contains('GXOS_NET10:MANAGED_HTTPS_PHASE42_RESOURCE_PASS') -and
                        !$finalText.Contains('GXOS_NET10:MANAGED_KERNEL_PHASE42_PASS') -and
                        !$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected malformed Phase 42 result."
        } elseif ($EnablePhase42Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE42') `
                "Boot $sequence did not complete the Phase 42 bounded HTML tokenizer proof: $phase15Outcome."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
        } elseif ($EnablePhase41Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE41') `
                "Boot $sequence did not complete the Phase 41 bounded text proof: $phase15Outcome."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
        } elseif ($EnablePhase40Protocol) {
            Require11 ($phase15Outcome -eq 'PASS_PHASE40') `
                "Boot $sequence did not complete the Phase 40 gzip resource proof: $phase15Outcome."
            Require11 (!$finalText.Contains('GXOS_NET10:CPU_EXCEPTION_VECTOR=') -and
                        !$finalText.Contains('GXOS_NET10:PAGE_FAULT_') -and
                        !$finalText.Contains('GXOS_NET10:UNEXPECTED_IMPORT_CALL:')) `
                "Boot $sequence reported an unexpected machine fault."
