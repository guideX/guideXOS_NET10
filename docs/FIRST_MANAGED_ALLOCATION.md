...
The 2026-07-30 `_initterm_e` pass also leaves allocation unproven. Its iterator performs no allocation, and the actual NativeAOT table contains one null entry, so no initializer callback ran. All three final QEMU summaries still report allocation-context pointer/limit `0/0`, `ALLOCATION_CONTEXT_VALID=0`, `MANAGED_THREAD_REGISTERED=0`, and no observable GC advancement. The new deepest boundary is `api-ms-win-crt-runtime-l1-1-0.dll!_initterm`; `_initterm` is a separate void-initializer contract and was not implemented.
