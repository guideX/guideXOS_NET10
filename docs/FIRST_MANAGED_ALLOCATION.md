...
The separate authentic NativeAOT startup path now proves FILETIME, QPC, and QPF, initializes the two empty CRT on-exit tables, initializes one aligned x64 SLIST header, completes the actual one-null `_initterm_e` range, and completes the nine-entry `_initterm` range. The path then stops at `api-ms-win-crt-string-l1-1-0.dll!strcmp`. These iterator and callback contracts are allocation-free and do not initialize a GC heap, allocation context, or managed thread. The first-allocation probe therefore remains `-10`; the next experiment must close the `strcmp` dependency and remaining NativeAOT PAL ownership contracts before retrying one allocation.
...
The 2026-07-30 `_initterm_e` and `_initterm` pass also leaves allocation unproven. `_initterm_e` skipped its one null entry; `_initterm` invoked and returned from all eight actual non-null callbacks. All three final QEMU summaries still report allocation-context pointer/limit `0/0`, `ALLOCATION_CONTEXT_VALID=0`, `MANAGED_THREAD_REGISTERED=0`, and no observable GC advancement. The new deepest boundary is `api-ms-win-crt-string-l1-1-0.dll!strcmp`.
