...
The 2026-07-29 SLIST evidence-closure pass does not change this allocation status. The SLIST implementation is allocation-free, and the complete final-artifact probe reports zero allocation-context pointer/limit, `ALLOCATION_CONTEXT_VALID=0`, no GC-advanced marker, and no managed-thread registration. The required three-run final-hash sequence remains open because its three QEMU processes were incomplete; the next boundary is still `_initterm_e`.
