...
| `api-ms-win-crt-string-l1-1-0.dll` | `strcmp (0x7d3c8)`; `strcpy_s (0x7d3d0)`; `strcpy (0x7d3d8)`; `_stricmp (0x7d3e0)`; `strlen (0x7d3e8)` | CRT/PAL configuration and diagnostics support; `strcmp` and `strlen` are reached in order after `_initterm`; `_stricmp` is the next boundary | No | `strcmp`, `strlen` | A for `strcmp`; B: fail-fast stubs for the other four | `objdump -p`; bounded call diagnostics; three immutable `strcmp`/`strlen` runs |
