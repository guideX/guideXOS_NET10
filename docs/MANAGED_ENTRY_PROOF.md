...
The historical allocation-enabled NativeAOT entry has a CRT opt-in profile with 22 functional imports and 102 deterministic fail-fast imports. After the already-proven FILETIME and QPC/QPF sequence, the attach helper calls `_initialize_onexit_table` twice. Each call returns zero and emits `CRT_ONEXIT_INITIALIZED_OK`; the historical next boundary is `KERNEL32.dll!InitializeSListHead`. The later register-enabled profile is a separate follow-on documented in [CRT_REGISTER_ONEXIT_FUNCTION_BOOTSTRAP.md](CRT_REGISTER_ONEXIT_FUNCTION_BOOTSTRAP.md). It reaches `_register_onexit_function`, returns `GROWTH_REQUIRED` / `-1` before `_recalloc_crt_t(_PVFV,NULL,0x20)`, and still does not prove managed allocation or GC startup. The serial traces retain `TLS_ALLOC_LIMIT=0`, `TLS_ALLOC_PTR=0`, `MANAGED_THREAD_REGISTERED=0`, and `ALLOCATION_CONTEXT_VALID=0`.
