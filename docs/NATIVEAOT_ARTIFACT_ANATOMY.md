...
## `_register_onexit_function` call anatomy (2026-08-02)

The current register-enabled payload imports `_register_onexit_function` at descriptor `8`, IAT RVA `0x7d358`, and preferred IAT `0x18007d358`. The only live call is preferred `0x180077e13`, within `NativeAOT_CRT_atexit_registration_helper` at `0x180077df0..0x180077e30`. The helper passes the initialized table at preferred RVA `0xb3e78` and the callback at preferred RVA `0x37bd0`; the loader records both preferred and relocated runtime addresses and proves the return site is six bytes after the call.

The table is the same first table initialized earlier by `_initialize_onexit_table`. Its three fields contain the same security-cookie-encoded null token, which decodes to `first=last=end=0`. The callback lies in the relocated executable managed-image text range. Static and dynamic call-site evidence is therefore tied to the actual imported function and actual table, not to a synthetic callback or unrelated loader object. Registration stops before `_recalloc_crt_t(_PVFV,NULL,0x20)`; no storage, callback slot, GC state, or managed allocation is fabricated.
