...
Gate 4 still proves the first no-allocation managed handoff. The allocation follow-on now builds and traces the generated allocation helper, but its authentic NativeAOT startup is bounded at `KERNEL32.dll!GetSystemTimeAsFileTime`, and the loader's 18 functional imports remain a bounded Gate 4 contract rather than implementation of the following runtime services.
...
| GC initialization | `Runtime.WorkstationGC.lib` and its `RhpNewFast` path are physically linked, but standard startup stops at an unprovided process-time import and no heap/segments/allocation context are initialized. | Implement the required NativeAOT PAL services or identify a supported freestanding runtime configuration; do not add dummy success shims. |
