...
Status: passed for the bounded `QueryPerformanceCounter` / `QueryPerformanceFrequency` milestone. The allocation-enabled NativeAOT startup path now returns from FILETIME, obtains a monotonic normalized counter value, and stops at the next authentic import, `api-ms-win-crt-runtime-l1-1-0.dll!_initialize_onexit_table`. No GC heap, managed allocation, or thread runtime is claimed.
...
The next real blocker is CRT on-exit/bootstrap initialization at `api-ms-win-crt-runtime-l1-1-0.dll!_initialize_onexit_table`. GC heap ownership, virtual memory, managed-thread registration, and first allocation remain separate unresolved contracts.
