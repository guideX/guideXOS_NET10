...
The current register follow-on closes only the separately scoped imported call's initial empty-table path. It allocates exactly `0x100` bytes through the existing UEFI `BootServices->AllocatePool(EFI_LOADER_DATA, 0x100)` primitive, stores 32 encoded callback slots, returns zero, and does not execute the callback. Nonempty growth, `_recalloc`, shutdown, and process-teardown behavior remain out of scope. See [CRT_REGISTER_ONEXIT_FUNCTION_BOOTSTRAP.md](CRT_REGISTER_ONEXIT_FUNCTION_BOOTSTRAP.md).
