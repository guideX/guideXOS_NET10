...
internal sealed class ManagedIpv4Layer : IManagedTcpPacketSender
{
    ...
    private ManagedPhase52HtmlProof? _phase52Consumer;
    private ManagedPhase53HtmlProof? _phase53Consumer;
    ...
    /* Phase 39 exercises the reusable resource layer against the same
       deterministic HTTPS fixture used by the host-facing TLS proofs.  The
       network setup remains identical to Phase 35; only the consumer above
       ManagedHttpsClient changes. */
    internal bool TryRunPhase39()
    {
        if (_phase39Passed || _active || _networkService == null)
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE39_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase39Consumer ??= new ManagedPhase39ResourceProof(_networkService);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE39_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE39_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_DHCP_COMPLETE\r\n"u8) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_DHCP_UNREGISTERED\r\n"u8) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver))
            return false;
        ...
        if (!KernelLog.Write(
            ...
            !KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE34_CONFIGURED\r\n"u8))
            return false;

        if (!KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_IPV4=0x"u8,
                _localIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_SUBNET=0x"u8,
                _subnetMaskValue) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_GATEWAY=0x"u8,
                _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE39_DNS=0x"u8,
                DnsServerValue))
            return false;

        ManagedNetworkServiceBackend.SetLiveIpv4(this);
        if (!_phase39Consumer.TryRun()) return false;
        _phase39Passed = true;
    ...
    /* Phase 41 proves the text layer above the same bounded HTTPS/resource
       topology.  The fixture is selected by the host runner and declares a
       UTF-8 text/plain representation. */
    internal bool TryRunPhase41()
    {
        if (_phase41Passed || _active || _networkService == null)
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE41_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase41Consumer ??= new ManagedPhase41TextProof(_networkService);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE41_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE41_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_DHCP_COMPLETE\r\n"u8) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_DHCP_UNREGISTERED\r\n"u8) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver))
            return false;
        if (!PublishNetworkServiceStatus()) return false;
        if (!KernelLog.Write(
            ...
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_IPV4=0x"u8,
                _localIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_SUBNET=0x"u8,
                _subnetMaskValue) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_GATEWAY=0x"u8,
                _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE41_DNS=0x"u8,
                DnsServerValue))
            return false;

        ManagedNetworkServiceBackend.SetLiveIpv4(this);
        if (!_phase41Consumer.TryRun()) return false;
        _phase41Passed = true;
        return KernelLog.Write(
            "GXOS_NET10:MANAGED_HTTPS_PHASE41_PASS\r\n"u8);
    }

    /* Phase 42 proves the bounded HTML tokenizer above the Phase 41 resource
       path.  It deliberately shares DHCP, DNS registration, TCP, TLS,
       decompression, MIME, and UTF-8 gates with the earlier proof. */
    internal bool TryRunPhase42()
    {
        if (_phase42Passed || _active || _networkService == null)
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE42_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase42Consumer ??= new ManagedPhase42HtmlProof(_networkService);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE42_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE42_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_DHCP_COMPLETE\r\n"u8) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_DHCP_UNREGISTERED\r\n"u8) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver))
            return false;
        if (!PublishNetworkServiceStatus()) return false;
        if (!KernelLog.Write(
            ...
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_IPV4=0x"u8,
                _localIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_SUBNET=0x"u8,
                _subnetMaskValue) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_GATEWAY=0x"u8,
                _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE42_DNS=0x"u8,
                DnsServerValue))
            return false;

        ManagedNetworkServiceBackend.SetLiveIpv4(this);
        if (!_phase42Consumer.TryRun()) return false;
        _phase42Passed = true;
        return KernelLog.Write(
            "GXOS_NET10:MANAGED_HTTPS_PHASE42_PASS\r\n"u8);
    }

    /* Phase 43 retains the bounded document arena after the Phase 42
       tokenizer.  It deliberately shares the same network and TLS proof
       sequence so the new managed tree is the only expanded surface. */
    internal bool TryRunPhase43(bool capacityControl = false)
    {
        if (_phase43Passed || _active || _networkService == null)
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE43_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase43Consumer ??= new ManagedPhase43HtmlProof(_networkService, capacityControl);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write(
                "GXOS_NET10:MANAGED_KERNEL_PHASE43_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE43_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_DHCP_COMPLETE\r\n"u8) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort))
            return false;
        if (!KernelLog.Write(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_DHCP_UNREGISTERED\r\n"u8) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver))
            return false;
        if (!PublishNetworkServiceStatus()) return false;
        if (!KernelLog.Write(
            ...
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_IPV4=0x"u8,
                _localIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_SUBNET=0x"u8,
                _subnetMaskValue) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_GATEWAY=0x"u8,
                _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine(
                "GXOS_NET10:MANAGED_HTTPS_PHASE43_DNS=0x"u8,
                DnsServerValue))
            return false;
    ...
    internal bool TryRunPhase48(bool capacityControl = false) =>
        TryRunPhase44Core(capacityControl, true, true, true);
    ...
    internal bool TryRunPhase52()
    {
        if (_phase52Passed || _active || _networkService == null)
        {
            KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE52_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase52Consumer ??= new ManagedPhase52HtmlProof(_networkService);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE52_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE52_DHCP_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver) ||
            !PublishNetworkServiceStatus() ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE52_CONFIGURED\r\n"u8) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE52_IPV4=0x"u8,
                                    _localIpv4Value) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE52_SUBNET=0x"u8,
                                    _subnetMaskValue) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE52_GATEWAY=0x"u8,
                                    _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE52_DNS=0x"u8,
                                    DnsServerValue))
            return false;
        ManagedNetworkServiceBackend.SetLiveIpv4(this);
        if (!_phase52Consumer.TryRun()) return false;
        _phase52Passed = true;
        return KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE52_PASS\r\n"u8);
    }

    internal bool TryRunPhase53()
    {
        if (_phase53Passed || _active || _networkService == null)
        {
            KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE53_IPV4_GUARD_FAILED\r\n"u8);
            return false;
        }
        _phase53Consumer ??= new ManagedPhase53HtmlProof(_networkService);
        if (!_arp.TryBeginDhcp())
        {
            KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE53_ARP_DHCP_BEGIN_FAILED\r\n"u8);
            return false;
        }
        _active = true;
        _networkService.BeginBoot();
        ...
        if (!_udpEndpoints.TryRegister(DhcpClientPort,
                                       ManagedUdpEndpointHandler.Dhcpv4Client) ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE53_DHCP_BEGIN\r\n"u8) ||
            !TryRunDhcpDora(requireDnsServer: true, requireGateway: false) ||
            !_udpEndpoints.TryUnregister(DhcpClientPort) ||
            !_udpEndpoints.TryRegister(DnsClientPort,
                                       ManagedUdpEndpointHandler.DnsResolver) ||
            !PublishNetworkServiceStatus() ||
            !KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE53_CONFIGURED\r\n"u8) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE53_IPV4=0x"u8,
                                    _localIpv4Value) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE53_SUBNET=0x"u8,
                                    _subnetMaskValue) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE53_GATEWAY=0x"u8,
                                    _gatewayIpv4Value) ||
            !KernelLog.WriteHexLine("GXOS_NET10:MANAGED_HTTPS_PHASE53_DNS=0x"u8,
                                    DnsServerValue))
            return false;
        ManagedNetworkServiceBackend.SetLiveIpv4(this);
        if (!_phase53Consumer.TryRun()) return false;
        _phase53Passed = true;
        return KernelLog.Write("GXOS_NET10:MANAGED_KERNEL_PHASE53_PASS\r\n"u8);
    }
                                   ...
                                   bool phase50Mode = false)
    {
        ...
        if (fontMode)
        {
            KernelLog.Write("GXOS_NET10:MANAGED_HTTPS_PHASE48_PASS\r\n"u8);
            return true;
        }
        if (paintMode)
        {
