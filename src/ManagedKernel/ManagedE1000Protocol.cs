...
internal static class ManagedE1000Protocol
{
    ...
    internal const uint Phase19RxTestSequence = 0x19000001;
    internal const uint Phase20RxTestSequence = 0x20000001;
    ...
    internal const uint Phase33RxTestSequence = 0x33000001;
    internal const uint Phase34RxTestSequence = 0x34000001;
                                                 ...
                                                 ReadOnlySpan<byte> destination)
    {
        ...
        if (sequence != RxTestSequence && sequence != Phase17RxTestSequence &&
            ...
            sequence != Phase32RxTestSequence && sequence != Phase33RxTestSequence &&
            sequence != Phase34RxTestSequence)
            return false;
    ...
    internal static bool IsPhase19RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase19RxTestSequence;
    }

    internal static bool IsPhase20RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase20RxTestSequence;
    }

    internal static bool IsPhase21RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase21RxTestSequence;
    }

    internal static bool IsPhase22RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase22RxTestSequence;
    }

    internal static bool IsPhase23RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
    ...
    internal static bool IsPhase33RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase33RxTestSequence;
    }

    internal static bool IsPhase34RxTestFrame(ReadOnlySpan<byte> frame)
    {
        if (frame.Length != RxFrameLength ||
            !frame.Slice(14, RxPayloadSignature.Length)
                .SequenceEqual(RxPayloadSignature)) return false;
        ...
        uint sequence = ((uint)frame[sequenceOffset] << 24) |
                        ...
                        ((uint)frame[sequenceOffset + 2] << 8) |
                        frame[sequenceOffset + 3];
        return sequence == Phase34RxTestSequence;
    }
